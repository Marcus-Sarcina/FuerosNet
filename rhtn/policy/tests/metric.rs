...
use rhtn_policy::conformance::{self, Rng, Row, Verdict, attach_fake_region, attach_region_behind, build_tree, run};
...
fn candidates_rank_by_flow_then_hops_then_consideration_order() {
    ...
    let a = allocate(&g);
    let ranked: Vec<(&str, u64, Option<usize>)> = a.ranked.iter().map(|r| (r.candidate.as_str(), r.flow, r.hops)).collect();
    // the fixture's expected per-candidate flows and hops
    assert_eq!(ranked, [("A", 2, Some(3)), ("B", 1, Some(2)), ("C", 1, Some(3)), ("D", 1, Some(4)), ("E", 1, Some(4))]);
...
fn the_three_passes_each_decide_their_own_case() {
    ...
    assert_eq!(flow_vs_distance(["A", "B"]), [s("A")], "pass 0: available flow outranks a shorter path");
    ...
    assert_eq!(unequal(["A", "B"]), [s("B")], "pass 1: the shorter path wins");
    ...
    for build in [["A", "B"], ["B", "A"]] {
        assert_eq!(equal(["A", "B"], build), [s("A")], "pass 2: the earlier-considered wins a true tie");
...
fn parallel_edges_between_one_pair_collapse_to_one_capacity() {
    ...
    assert_eq!(ev.pairs().iter().filter(|(a, b)| (a, b) == (&s("h2"), &s("p"))).count(), 1, "one pair, one edge");
    assert_eq!(m.graph(&ev).capacity(&s("h2"), &s("p")), m.edge_capacity);
    assert_eq!(m.flow(&ev, &s("p")), m.edge_capacity, "the computed flow is the single-edge value");
    ...
    for u in summed.nodes() {
        for (v, c) in summed.edges_from(u) {
            let c = if (u.as_str(), v.as_str()) == ("h2", "p") || (u.as_str(), v.as_str()) == ("p", "h2") { 3 * c } else { c };
            g.add_edge(u.clone(), v.clone(), c);
        }
    }
    summed = g;
    assert_eq!(landscape::score(&summed, &s("o"), &s("p"), Some(&ev.scope())), 3 * m.edge_capacity, "summing would give a larger flow");
}
...
fn the_aggregate_a_set_can_draw_is_bounded_by_the_cut_per_computation() {
    ...
    assert_eq!(rows, [(4, 32, 4, 8), (8, 64, 8, 8), (16, 104, 8, 8), (32, 168, 8, 8)]);
    assert_eq!(rows[2].2, rows[3].2, "the joint stops growing once the region exceeds its cut");
    ...
    assert!(asked > m.ceiling(&ev, &gate), "vacuous unless demand exceeds the cut");
    assert_eq!((asked, delivered), (168, 8));
    // and the figures are the simulation's committed ones
    let committed = std::fs::read_to_string(concat!(env!("CARGO_MANIFEST_DIR"), "/../../models/results/flow_metric.txt")).expect("the committed simulation results");
    for (width, independent, joint, cut) in rows {
        let line = committed.lines().find(|l| l.contains(&format!("{width:>2} identities behind ONE acquired edge"))).unwrap_or_else(|| panic!("no committed row for {width}"));
        assert!(line.contains(&format!("independent-sum = {independent:>4}")), "{line}");
        assert!(line.contains(&format!("conserving joint = {joint}")), "{line}");
...
fn setwise_conservation_holds_per_computation_not_per_lifetime() {
    ...
    assert!(joint <= cut);
    assert!(solo_a + solo_b > joint, "separate admission draws more than the cut allows");
}
...
fn visibility_does_not_compose() {
    ...
    for reach in 0..=3 {
        ...
        assert!(ev.knows(&s("G")), "the visible edge brings its far endpoint in");
        assert!(!ev.knows(&s("X")), "a peering edge with no in-horizon endpoint became visible at reach {reach}");
        assert_eq!(m.flow(&ev, &s("X")), 0, "an invisible edge carried standing at reach {reach}");
    }
}
...
fn hops_in_the_collapsed_graph_are_the_landscape_distance() {
    ...
    assert!(checked > fakes.len(), "the whole graph was checked, region included");
...
fn the_bound_has_the_horizon_boundary_as_its_shape() {
    ...
    assert!(ev.horizon().contains("FAKE"), "a direct child's adoptee is inside the horizon");
    let best = fakes.iter().map(|f| m.flow(&ev, f)).max().unwrap();
    assert_eq!(best, UNTHROTTLED, "inside the horizon the metric does not ration; if this bounds, the horizon is throttled somewhere it should not be");
    ...
    assert!(fakes.iter().all(|f| !ev.knows(f)), "identities beyond an invisible edge leaked in");
...
fn the_conformance_test_reports_a_policy_s_resistance_bound() {
    ...
    for region in [&reference.cut_bound, &reference.conservation] {
        ...
        for Row { identities, best, joint, .. } in &region.rows {
            assert!(*best <= cut as f64 && *joint <= cut as f64, "{identities} identities: best {best}, joint {joint}, cut {cut}");
        }
    }
    let sizes: Vec<usize> = reference.cut_bound.rows.iter().map(|r| r.identities).collect();
    assert_eq!(sizes, [7, 40, 341], "the simulation's population sizes");
    assert_eq!(reference.conservation.rows.iter().map(|r| r.identities).collect::<Vec<_>>(), [4, 8, 16, 32]);
...
fn the_branching_condition_is_necessary_and_not_sufficient() {
    ...
    assert!(series::fake_mass(lam, 1, |_| f as f64, 200) / honest < 21.0, "at branching f the series converges to 1/(1 - fλ) = 20");
    assert!(series::fake_mass(lam, 1, |_| (f + 1) as f64, 200) / honest > 1000.0, "at branching f + 1 the same λ diverges");
    assert_eq!(series::classify(lam, |_| f as f64, 200), Convergence::Converges);
    assert_eq!(series::classify(lam, |_| (f + 1) as f64, 200), Convergence::Diverges);
    assert_eq!(series::classify(lam, |i| (f + i as u64) as f64, 200), Convergence::Diverges, "an acquaintance degree growing with depth");
    ...
    assert_eq!((b.fanout, b.lambda, b.criterion_satisfied), (f, Some(lam), Some(true)));
    assert_eq!(b.hierarchy_verdict, Convergence::Converges, "bounded acquaintance degree: {:?}", b.hierarchy_only);
    assert_eq!(b.acquaintance_verdict, Convergence::Diverges, "acquaintance degree growing with depth: {:?}", b.with_acquaintance);
    // an implementation reporting the satisfied criterion as sufficient
    // would have said the second converges
    assert!(!(b.criterion_satisfied == Some(true) && b.acquaintance_verdict == Convergence::Converges));
    ...
    assert_eq!((reference.hierarchy_verdict, reference.acquaintance_verdict), (Convergence::Converges, Convergence::Converges));
    assert!(reference.with_acquaintance.iter().all(|j| *j == 8.0), "{:?}", reference.with_acquaintance);
}

#[test]
fn a_scope_is_adoption_and_sibling_edges_only() {
    let scope = Scope::from_adoptions(&[(s("p"), s("a")), (s("p"), s("b")), (s("a"), s("c"))]);
    assert_eq!(scope.horizon(&s("c"), 2), ["c", "a", "p", "b"].map(s).into_iter().collect());
    assert_eq!(scope.horizon(&s("b"), 1), ["b", "p", "a"].map(s).into_iter().collect(), "siblings are one scope edge apart");
    ...
    assert_eq!((d["b"], d["p"], d["a"], d["q"]), (0, 1, 1, 1), "a counterparty met is at 1, in the horizon or not");
}
