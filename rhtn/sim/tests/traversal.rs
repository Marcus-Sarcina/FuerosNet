...
async fn scene(mapping: Mapping, filtering: Filtering) -> Scene {
    ...
    let mut v1 = view_of("alice", table_of("alice", &s, &records, &infra), "alice", &[], s.clock);
    v1.set_slot(1, Some(kh("bob")), s.clock);
    v1.set_slot(0, Some(kh("carol")), s.clock);
    let s1 = LiveNode::start(node_cfg("alice", I), v1, ids(), AnchorTable::new(0, Ingestion::UnverifiedGossip));
    let mut v2 = view_of("bob", table_of("bob", &s, &records, &infra), "alice", &[1], s.clock);
    v2.set_slot(0, Some(kh("w1")), s.clock);
    let s2 = LiveNode::start(node_cfg("bob", I), v2, ids(), AnchorTable::new(0, Ingestion::UnverifiedGossip));
    ...
    let mut lv1 = view_of("carol", table_of("carol", &s, &records, &infra), "alice", &[0], s.clock);
    lv1.serving_node = Some(kh("alice"));
    let l1 = LiveNode::start(c1, lv1, ids(), AnchorTable::new(0, Ingestion::UnverifiedGossip));
    let mut c2 = node_cfg("w1", I);
    c2.nat = Some(nat2.inside);
    let mut lv2 = view_of("w1", table_of("w1", &s, &records, &infra), "alice", &[1, 0], s.clock);
    lv2.serving_node = Some(kh("bob"));
    let l2 = LiveNode::start(c2, lv2, ids(), AnchorTable::new(0, Ingestion::UnverifiedGossip));
    ...
    let AttachOutcome::Attached(l1_up) = l1.attach_upstream(&cfg1, kh("alice")).await else { panic!("L1 attaches to S1") };
    let cfg2 = client_cfg("w1");
    know(&cfg2, "bob", s2.addr);
    let AttachOutcome::Attached(l2_up) = l2.attach_upstream(&cfg2, kh("bob")).await else { panic!("L2 attaches to S2") };
    Scene { s, s1, s2, l1, l2, l1_up, l2_up, nat1, nat2 }
}
...
fn relay(sc: &Scene, to: &str, bytes: Vec<u8>) {
    ...
    node.node.enqueue(kh(to), bytes).expect("the serving node relays");
}

async fn next_delivery(s: &mut Session) -> Option<Vec<u8>> {
    tokio::time::timeout(Duration::from_secs(3), s.deliveries.recv()).await.ok().flatten()
}
...
async fn candidates_carry_the_host_address_and_the_reflexive_address_the_serving_node_saw() {
    ...
    assert_eq!(sc.s1.traversal.answered.load(Ordering::SeqCst), 0, "nothing asked yet");
    let cs = sc.l1.gather().await;
    assert_eq!(sc.l1.traversal.asked.load(Ordering::SeqCst), 1, "one Binding Request to S1");
...
async fn exchange_and_open(sc: &mut Scene) -> (bool, bool) {
    ...
    let got2 = decode_candidates(&next_delivery(&mut sc.l2_up).await.expect("L2 receives L1's candidates")).unwrap();
    let got1 = decode_candidates(&next_delivery(&mut sc.l1_up).await.expect("L1 receives L2's candidates")).unwrap();
    assert_eq!((got1, got2), (c2.clone(), c1.clone()));
    let p = pins();
    let (a, b) = tokio::join!(sc.l1.open_direct(kh("w1"), &p, &c2), sc.l2.open_direct(kh("carol"), &p, &c1));
    (a, b)
}
...
async fn the_direct_path_is_attempted_first_and_the_relay_taken_when_the_checks_fail() {
    ...
    let (sender, receiver, inbox) = if a { (&sc.l1, "w1", &mut inbox2) } else { (&sc.l2, "carol", &mut inbox1) };
    let relayed = std::cell::Cell::new(0);
    let d = sender.send_payload_live(kh(receiver), b"direct".to_vec(), &|_| relayed.set(relayed.get() + 1)).await;
    assert_eq!(d, LiveDelivery::Direct);
    let (from, bytes) = tokio::time::timeout(Duration::from_secs(3), inbox.recv()).await.unwrap().unwrap();
    assert_eq!((from, bytes.as_slice()), (sender.me(), &b"direct"[..]));
    assert_eq!(relayed.get(), 0);
    assert_eq!(sc.s1.node.queued(&kh("carol")) + sc.s2.node.queued(&kh("w1")), 0, "neither serving node carries the bytes");
    drop(sc);
    // address-and-port-dependent NATs: the checks fail within the bound and the relay carries it
    let mut sc = scene(Mapping::AddressAndPortDependent, Filtering::AddressAndPortDependent).await;
    let (a, b) = exchange_and_open(&mut sc).await;
    assert!(!a && !b, "no candidate pair connects");
    assert_eq!(sc.l1.direct_state(&kh("w1")), Some(false));
    let d = sc.l1.send_payload_live(kh("w1"), b"relayed".to_vec(), &|bytes| relay(&sc, "w1", bytes)).await;
    assert_eq!(d, LiveDelivery::Relayed);
    assert_eq!(next_delivery(&mut sc.l2_up).await.as_deref(), Some(&b"relayed"[..]), "delivered through L2's serving node");
}
...
async fn the_relay_carries_every_message_in_order_while_the_direct_path_stays_defeated() {
    ...
    for i in 0..5 {
        let d = sc.l1.send_payload_live(kh("w1"), format!("m{i}").into_bytes(), &|bytes| relay(&sc, "w1", bytes)).await;
        assert_eq!(d, LiveDelivery::Relayed);
    }
    assert!(started.elapsed() < Duration::from_secs(1), "no send waited on a retry of the direct path");
    for i in 0..5 {
        assert_eq!(next_delivery(&mut sc.l2_up).await, Some(format!("m{i}").into_bytes()), "in order");
    }
    assert_eq!(sc.l1.traversal.asked.load(Ordering::SeqCst), asked, "no gathering between sends");
    assert_eq!(sc.l1.direct_state(&kh("w1")), Some(false), "the failure stands until the next opening");
}

// acceptance: TRV-04
#[tokio::test]
async fn control_traffic_dials_outward_and_never_traverses() {
    let sc = scene(Mapping::AddressAndPortDependent, Filtering::AddressAndPortDependent).await;
    ...
    sc.l1.view.lock().unwrap().require_currency(&sc.l1.adjacency, &ids(), &kh("w1"), None, Some(kh("alice")));
    tokio::time::sleep(Duration::from_millis(300)).await;
    assert_eq!(sc.l1.traversal.asked.load(Ordering::SeqCst), 0, "no STUN request left L1");
    assert_eq!(sc.s1.traversal.answered.load(Ordering::SeqCst), 0, "S1 answered none");
    assert_eq!(sc.l2.traversal.asked.load(Ordering::SeqCst) + sc.s2.traversal.answered.load(Ordering::SeqCst), 0);
    assert!(sc.nat1.forwarded.load(Ordering::SeqCst) > 0, "the session itself crossed the NAT on the outward dial");
}
...
async fn nothing_is_gathered_and_no_address_dialled_for_a_peer_outside_the_horizon() {
    ...
    let far = kh("w2");
    assert!(!sc.l1.view.lock().unwrap().table.horizon(&kh("carol"), 2).contains(&far));
    assert!(sc.l1.prepare_direct(&far).await.is_none(), "no candidates for a peer outside the horizon");
    assert_eq!(sc.l1.traversal.asked.load(Ordering::SeqCst), 0, "no STUN request");
...
async fn the_reflexive_addresses_travel_only_between_the_two_leaves() {
    ...
    let (a, b) = exchange_and_open(&mut sc).await;
    assert!(a || b);
    let refl: Vec<Vec<u8>> = [&sc.nat1, &sc.nat2].iter().flat_map(|n| n.mappings()).map(|a| match a.ip() { std::net::IpAddr::V4(ip) => { let mut v = ip.octets().to_vec(); v.extend_from_slice(&a.port().to_be_bytes()); v } _ => unreachable!() }).collect();
    tokio::time::sleep(Duration::from_millis(300)).await;
    for node in [&sc.s1, &sc.s2] {
        let replicated: Vec<Vec<u8>> = node.replication_payload().into_iter().map(|r| format!("{r:?}").into_bytes()).collect();
        let v = node.view.lock().unwrap();
        let mut state: Vec<Vec<u8>> = v.store.objects().into_iter().map(|(_, o)| o).collect();
        state.extend(v.own_endpoints().into_iter().map(|p| format!("{p:?}").into_bytes()));
        state.extend(replicated);
        for bytes in &state {
            for r in &refl {
                assert!(!bytes.windows(r.len()).any(|w| w == r.as_slice()), "a reflexive address in topology state at a serving node");
            }
        }
    }
