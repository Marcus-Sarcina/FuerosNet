...
fn entry(owner: &str, resource: [u8; 32], service_type: &str, endpoint: &[u8], practice: Option<u64>) -> Vec<u8> {
    CatalogEntry::build(&id(owner), &EntryFields { resource, service_type: service_type.into(), instance: "x".into(), endpoint: endpoint.to_vec(), connect_scope: None, metadata: None, data_practice: practice })
}
...
fn a_repeated_continuation_is_truncation_not_another_page() {
    ...
    let page: Vec<Vec<u8>> = (0..111u32).map(|i| entry("bob", rhtn_codec::cose::sha256(&i.to_be_bytes()), "rhtn-forum", b"e", None)).collect();
    let answer = |q: &CatalogQuery| CatalogReply { nonce: q.nonce, entries: page.clone(), continuation: Some("rhtn-forum".into()) };
    let q1 = sweep.query(None, [1; 16]);
    let step = sweep.take(&ids(), view.portion(node), &answer(&q1), 111);
    assert_eq!(step, Step::Again("rhtn-forum".into()), "the first hint is followed");
    ...
    assert_eq!(step, Step::Truncated, "a full page and a hint already asked for");
    assert!(view.portion(node).truncated, "H's portion marked truncated");
    assert_eq!(sweep.queries, 2, "asked for that type at most once more, then stopped");
    ...
    assert_eq!(s2.take(&ids(), view.portion(kh("w2")), &CatalogReply { nonce: q.nonce, entries: page[..3].to_vec(), continuation: None }, 111), Step::Done);
...
fn a_brokered_service_is_routed_to_only_where_it_matches_the_signed_entry() {
    let e = CatalogEntry::parse(&entry("bob", [7; 32], "saas-tracker", b"https://x.example/", None)).unwrap();
    assert_eq!(route_brokered(&e, b"https://y.example/"), Err(Refusal::EndpointDiffers { signed: b"https://x.example/".to_vec(), presented: b"https://y.example/".to_vec() }), "no traffic to Y");
    let r = route_brokered(&e, b"https://x.example/").unwrap();
    assert_eq!((r.endpoint.as_slice(), r.owner), (&b"https://x.example/"[..], kh("bob")), "the owner named in the entry is shown");
    // the page: what the node served, with the roles held; nothing re-expanded
    let served = vec![(entry("bob", [7; 32], "saas-tracker", b"https://x.example/", Some(0)), vec!["member".to_string()])];
    let shown = page(&ids(), &served);
    assert_eq!(shown.len(), 1);
    assert_eq!((shown[0].entry.resource, shown[0].roles.clone()), ([7; 32], vec!["member".to_string()]));
...
fn an_unrecognised_declaration_is_surfaced_and_a_known_one_is_not() {
    ...
    assert_eq!(hook.0.borrow().as_slice(), &[Notice::UnrecognisedDeclaration { resource: [1; 32], value: 9 }]);
