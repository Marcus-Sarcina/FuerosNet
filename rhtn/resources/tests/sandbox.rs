...
fn a_package_reaching_past_the_two_bindings_is_not_admitted() {
    ...
    for elsewhere in [
        ...
        match Sandbox::admit(&reaching(elsewhere), Limits::default()) {
            Err(Refusal::UnknownImport(n)) => assert_eq!(n, elsewhere, "the refusal names what was asked for"),
            other => panic!("{elsewhere} is not a binding this host has: {}", refusal(&other)),
        }
    }
    ...
    for hook in ["topology", "queue", "peers"] {
        match Sandbox::admit(&overreaching(hook), Limits::default()) {
            Err(Refusal::UnknownImport(n)) => assert_eq!(n, format!("{HOST_INSTANCE}#{hook}"), "the hook is named, not just the instance"),
            other => panic!("a hook inside the host's own instance is still a hook that does not exist: {}", refusal(&other)),
        }
    }

    assert!(matches!(Sandbox::admit(&mute(), Limits::default()), Err(Refusal::NoEntry)), "a package the host cannot call is not a package");
    assert!(
        matches!(Sandbox::admit(b"\0asm\x01\0\0\0", Limits::default()), Err(Refusal::NotAComponent(_))),
...
fn a_package_is_handed_its_request_and_gives_back_its_answer() {
    ...
    assert_eq!(back, message, "the package sees the message the node built and nothing else");
...
fn a_package_that_will_not_stop_is_stopped_and_costs_the_next_caller_nothing() {
    let limits = Limits { fuel: 1_000_000, ..Limits::default() };
    let s = Sandbox::admit(&spinning(), limits).expect("admitted");
    assert!(matches!(s.serve(b"GET / HTTP/1.1\r\n\r\n"), Err(Refusal::Exhausted)), "a loop is stopped rather than run");
    assert!(matches!(s.serve(b"GET / HTTP/1.1\r\n\r\n"), Err(Refusal::Exhausted)), "and stopped again, with its own budget");

    // a package that breaks is a different fact from one that spends
    let b = Sandbox::admit(&broken(), Limits::default()).expect("admitted");
    assert!(matches!(b.serve(b"GET / HTTP/1.1\r\n\r\n"), Err(Refusal::Trapped(_))), "a trap is a defect, not capacity");
...
fn a_package_that_grows_without_bound_is_bounded() {
    let limits = Limits { memory: 2 << 20, ..Limits::default() };
    let s = Sandbox::admit(&greedy(), limits).expect("admitted");
    let back = s.serve(b"GET / HTTP/1.1\r\n\r\n").expect("it stops rather than trapping");
    let pages = u32::from_le_bytes(back[..4].try_into().unwrap()) as usize;
    assert!(pages * 65536 <= limits.memory, "it reached {pages} pages, past the {} byte ceiling", limits.memory);
    assert!(pages > 1, "and it was allowed to grow at all: {pages} pages");

    // an answer larger than the host carries is refused as an answer, not
    // reported as a defect in the package
    let loud = Sandbox::admit(&shouting(4096), Limits { response: 1024, ..Limits::default() }).expect("admitted");
    assert!(matches!(loud.serve(b"GET / HTTP/1.1\r\n\r\n"), Err(Refusal::Oversized(4096))), "the host says how big it was");
}
...
fn a_stopped_package_is_unavailable_rather_than_broken() {
    ...
    assert!(h.running());
    assert_eq!(h.handle(b"GET / HTTP/1.1\r\n\r\n").expect("served"), b"GET / HTTP/1.1\r\n\r\n");
    h.stop();
    assert!(!h.running(), "the gateway asks before it hands anything over");
    assert!(h.handle(b"GET / HTTP/1.1\r\n\r\n").is_err(), "and a request that reaches it anyway is refused");
}
...
fn what_reaches_a_package_through_the_gateway_is_the_credential_and_the_request() {
    ...
    let hosted = Arc::new(Hosted::new(Sandbox::admit(&echo(), Limits::default()).expect("admitted")));
    g.bind(resource, Binding { owner: me, authority: "shop.internal".into(), backend: Some(hosted), declared_roles: BTreeSet::from(["reader".to_string()]) });
    g.set_row(resource, me, Row { roles: BTreeSet::from(["reader".to_string()]), connect: true }).expect("a row");

    let req = ResourceRequest { resource, message: b"GET /orders HTTP/1.1\r\nhost: ignored\r\naccept: application/json\r\n\r\n".to_vec() };
    ...
    assert!(body.starts_with("GET /orders HTTP/1.1"), "with its own request underneath");
    for absent in ["rhtn-topology", "rhtn-liveness", "rhtn-queue", "rhtn-patron", "rhtn-keyhash"] {
        assert!(!body.contains(absent), "and nothing about the network: {absent}");
    }
}
