...
fn host(name: &'static str, cfg: Config, serving: &Arc<dyn Serving>, inboxes: &Inboxes, verifiers: &Arc<Verifiers>) -> Party {
    let reachable = Reachable::default();
    let handle = spawn_client(name, cfg, reachable.clone());
    let (courier, app) = Courier::new(handle.clone(), serving.clone(), Arc::new(NoDirect(reachable)));
    inboxes.host(kh(name), courier.inbound());
    verifiers.host(handle.clone(), courier.clone());
    Party { handle, courier, app }
}
...
fn query(subject: &str, querier: &str, verifier: &str, ceremony: u8) -> VerificationQuery {
    VerificationQuery { subject: kh(subject), querier: kh(querier), ceremony_id: [ceremony; 32], profile: vec![1, 2, 3], template_version: 1, verifier: kh(verifier) }
}
...
async fn a_query_is_answered_at_the_node_hosting_the_verifier_and_the_subject_gets_its_copy() {
    ...
    let body = QueryRequest { query: q.clone(), consent: signed, selection_basis: 1 }.encode();
    ...
    let t0 = Instant::now();
    let reply = c.request(REQUEST_VERIFIER_QUERY, &body).await.expect("answered on the stream");
    // no grant within the bound: unavailable, signed by bob, on carol's stream
    let r = Response::read(&reply).unwrap();
    assert_eq!((r.verifier, r.subject, r.query_id, r.verdict), (kh("bob"), kh("alice"), q.query_id(), Verdict::Unavailable));
    verify::response(&ids(), &reply, false).expect("bob's signature");
    assert!(t0.elapsed() >= Duration::from_millis(400), "the buffer bound passed first");
    ...
    assert!(matches!(d, Dispatched::ResponseCopy(Ok(qid)) if qid == q.query_id()), "{d:?}");
    // a verifier nobody here hosts: the stream fails, nothing signed
    let q2 = VerificationQuery { verifier: kh("w1"), ..q.clone() };
    let body2 = QueryRequest { query: q2.clone(), consent: consent(&id("alice"), &q2.query_id()), selection_basis: 1 }.encode();
    ...
    let q3 = VerificationQuery { querier: kh("w2"), ceremony_id: [8; 32], ..q.clone() };
    let body3 = QueryRequest { query: q3.clone(), consent: consent(&id("alice"), &q3.query_id()), selection_basis: 1 }.encode();
    assert!(c.request(REQUEST_VERIFIER_QUERY, &body3).await.is_err());
    assert!(next(&mut alice.app, 300).await.is_none(), "no copy for a query never answered");
}
...
async fn a_grant_on_the_payload_channel_answers_the_waiting_stream_of_a_hosted_light_client() {
    ...
    let (consent, grant) = w1
        ...
        .await
        .expect("w1 consents");
    assert!(grant.is_none(), "w1 holds no capture alice sealed, so no grant travels with the consent");
    let body = QueryRequest { query: q, consent, selection_basis: 1 }.encode();
    let ccfg = client_cfg("carol");
    know(&ccfg, "bob", node.addr);
    let AttachOutcome::Attached(c) = attach(&ccfg, &client_ep(), kh("bob"), node.addr, false).await else { panic!("carol attaches") };
    let t0 = Instant::now();
    ...
    assert!(until(2000, || alice.handle.with_blocking(|c| c.verifier.awaiting()) == vec![qid]).await);
    ...
    let reply = tokio::time::timeout(Duration::from_secs(3), stream).await.expect("answered within the bound, not at it").unwrap().expect("answered on the stream");
    assert!(t0.elapsed() < Duration::from_secs(5));
    let r = Response::read(&reply).unwrap();
    assert_eq!((r.verifier, r.subject, r.verdict, r.basis), (kh("alice"), kh("w1"), Verdict::Unavailable, None), "the record named is not held: unavailable, nothing compared");
