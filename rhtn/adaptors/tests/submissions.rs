...
fn node_with(name: &str, tweak: impl FnOnce(&mut NodeConfig, &mut NodeView)) -> Arc<LiveNode> {
    ...
    let mut view = NodeView::new(Arc::new(id(name)), Locator { anchor: kh(name), path: p.bytes, nibbles: p.nibbles, seqno: Seqno { series: 1, counter: 0 } });
...
async fn attach_light(name: &str, node: &Arc<LiveNode>) -> Light {
    ...
    let AttachOutcome::Attached(mut s) = attach(&cfg, &client_ep(), node.me(), node.addr, false).await else { panic!("{name} attaches") };
    ...
    Light { serving, session, deliveries, me: kh(name) }
}
...
async fn a_publication_is_taken_only_from_the_subject_the_bundle_names() {
    ...
    // sent it
    let n1 = [9u8; 16];
    let alices = PrekeyPublication { bundle: bundle("alice"), nonce: n1 }.encode();
    assert_eq!(code(&carol, REQUEST_PREKEY_PUBLICATION, n1, alices).await, Some(SUBMISSION_REFUSED));
    assert!(node.view.lock().unwrap().prekeys.bundle(&kh("alice")).is_none(), "nothing held for alice");
    // carol's own is taken
    let n2 = [10u8; 16];
    let own = PrekeyPublication { bundle: bundle("carol"), nonce: n2 }.encode();
    assert_eq!(code(&carol, REQUEST_PREKEY_PUBLICATION, n2, own).await, Some(SUBMISSION_ACCEPTED));
...
async fn a_deposit_past_the_nodes_bound_is_refused_whole() {
    ...
    let carol = attach_light("carol", &node).await;
    let n1 = [1u8; 16];
    assert_eq!(code(&carol, REQUEST_ONE_TIME_DEPOSIT, n1, OneTimeDeposit { keys: keys(3), nonce: n1 }.encode()).await, Some(SUBMISSION_ACCEPTED));
    // three more would make six, past the pool it lends
    let n2 = [2u8; 16];
    assert_eq!(code(&carol, REQUEST_ONE_TIME_DEPOSIT, n2, OneTimeDeposit { keys: keys(3), nonce: n2 }.encode()).await, Some(SUBMISSION_OVER_BOUND));
    assert_eq!(node.view.lock().unwrap().prekeys.pool_size(&carol.me), 3, "the pool is what it was: nothing was trimmed in");
    // and one deposit larger than the per-deposit bound, whatever the pool
    // holds, is refused on its own count
    let n3 = [3u8; 16];
    assert_eq!(code(&carol, REQUEST_ONE_TIME_DEPOSIT, n3, OneTimeDeposit { keys: keys(5), nonce: n3 }.encode()).await, Some(SUBMISSION_OVER_BOUND));
...
async fn a_relay_submission_is_answered_on_taking_and_collected_later() {
    ...
    let delivered = tokio::time::timeout(std::time::Duration::from_secs(3), w1.deliveries.recv()).await.expect("delivered").expect("bytes");
    assert_eq!(unrelayed(&delivered), Some((carol.me, b"sealed".to_vec())));
    assert_eq!(node.node.queued(&kh("w1")), 0, "no copy outlives the delivery");
}
...
async fn one_wake_endpoint_per_relationship_replaced_on_re_registration() {
    let node = node_with("bob", |_, _| {});
    let carol = attach_light("carol", &node).await;
    let e1 = WakeEndpoint { url: "https://push.example/one".into(), key: vec![1; 32], lapses_at: None };
    let e2 = WakeEndpoint { url: "https://push.example/two".into(), key: vec![2; 32], lapses_at: Some(1_900_000_000) };
    ...
    assert_eq!((held.url.as_str(), held.key.as_slice(), held.lapses_at), (e2.url.as_str(), e2.key.as_slice(), e2.lapses_at), "the second replaced the first");
    assert_eq!(view.wake.holders(), vec![carol.me], "and only carol's is held");
}
...
async fn a_registration_with_no_endpoint_withdraws_and_a_key_without_one_is_malformed() {
    let node = node_with("bob", |_, _| {});
    let carol = attach_light("carol", &node).await;
    let e = WakeEndpoint { url: "https://push.example/one".into(), key: vec![1; 32], lapses_at: None };
    ...
    assert!(node.view.lock().unwrap().wake.get(&carol.me).is_none(), "forgotten");
    // a key with no endpoint to attach it to never reaches the register:
    // the decoder refuses the shape and the stream fails
    let n = [4u8; 16];
    let orphan = WakeRegistration { nonce: n, endpoint: None, key: Some(vec![3; 32]), lapses_at: None }.encode();
    assert!(WakeRegistration::decode(&orphan).is_err(), "the decoder refuses it here too");
    assert!(carol.session.request(REQUEST_WAKE, &orphan).await.is_err(), "and at the node");
...
async fn every_reply_echoes_its_own_nonce_and_says_nothing_else() {
    ...
    for (rt, nonce, body) in sent {
        ...
        assert_eq!(r.nonce, nonce, "each reply echoes the nonce of the request that drew it");
        assert!(r.code <= 2);
        // two fields and no more: the reply carries the nonce and the code
        let rhtn_codec::cbor::Item::Map(ref m) = rhtn_codec::cbor::parse_all(&bytes).unwrap() else { panic!("a map") };
        assert_eq!(m.len(), 2, "nothing further");
        codes.push(r.code);
    }
    assert_eq!(codes, vec![SUBMISSION_ACCEPTED, SUBMISSION_OVER_BOUND, SUBMISSION_REFUSED, SUBMISSION_ACCEPTED], "one of each answer, and no reason with any of them");
