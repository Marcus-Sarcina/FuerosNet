...
| **Cryptographic implementation audit** | **Not happening, and the plan should not pretend otherwise** [author, 2026-09-26]. A paid expert review was named here and will not be commissioned. The consequence is that this project's own cryptographic code is **permanently unreviewed rather than awaiting review**, which is why the payload session's construction is taken from libsignal in the reference clients (`implementation-plan.md` section 7) rather than trusted to the implementation here. LLM review is not a substitute and is not offered as one. What remains available and is worth doing: the hygiene an auditor would charge to report — key material wiped rather than left in freed memory, secrets not written to disk in the clear — and differential testing against a reference implementation, which establishes agreement and says nothing about implementation security |
