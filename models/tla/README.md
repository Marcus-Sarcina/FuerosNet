...
- `CycleDetection`: authority cycles resolve (`CyclesResolve`), on three nodes and on four, where a cycle node has a child that is not on the cycle and repair cuts the subordinate that handed the memo over; a memo whose stated patron no longer matches the detector's row does not fire.
