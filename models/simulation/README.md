...
- That an implementation retains no standing between evaluations. The metric is a pure function of a graph, and §16.2 now says outright that *"nothing here is a retained entitlement"* — the bound holds for the set an evaluation scores. A policy that keeps prior results and scores the next batch afresh is outside that rule, which the conservation regression exhibits (8 + 8 separately against a cut of 8) and cannot prevent. The design's one materialised consumer, §11.4's role table, is not such a policy: it scores Dunbar Org members inside the horizon, where the metric does not ration, so its rows spend no cut (§16.2).
