...
| ID | Kind | Requirement and functional test oracle | Authority |
|---|---|---|---|
| TOP-011 | S | Dispatch TopologyPush kind 0 as a signed transaction and kind 1 as an EndpointRecord, preserving the enclosed bytes. Reject malformed known push bodies and unsupported kind values with the specified frame-level behavior. | W §§8.2, 10.1 |
...
| SCH-021 | S | Cover TopologyPush kind/encoded object and TopologyMemo patron/locator/slot/source timestamp/optional occupant. Control type numbers are 1 Attach, 2 Ack, 3 Heartbeat, 4 SiblingUpdate, 5 Push, 6 Memo; numeric spaces are not interchangeable with transaction/request tags. | W §§8.0–8.2, 10 |
