...
fn a_complete_configuration_reads_and_a_missing_key_has_no_default() {
    ...
    for key in [
        ...
        assert!(
            e.what.contains(key) && e.what.contains("no default"),
            "{key}: {e}"
        );
    }
    let e = Config::parse(&GOOD[..GOOD.find("[allowance]").unwrap()]).unwrap_err();
    assert!(
        e.what.contains("allowance") && e.what.contains("no default"),
        "a table with no default either: {e}"
    );
}
...
fn an_unknown_or_repeated_key_is_refused_naming_its_line() {
    ...
    assert!(
        e.what.contains("listne") && e.what.contains("unknown field"),
        "{e}"
    );
    assert!(
        e.what.contains("identity"),
        "and the keys it did expect: {e}"
    );
    let e = Config::parse(&with("heartbeat = 30", "heartbeat = 30\nheartbeat = 60")).unwrap_err();
    assert!(
        e.what.contains("duplicate key") && e.what.contains("heartbeat"),
        "{e}"
    );
...
fn a_value_outside_what_the_wire_allows_is_refused() {
    let heartbeat = |v: &str| {
        Config::parse(&with("heartbeat = 30", &format!("heartbeat = {v}")))
            .map(|c| c.heartbeat_secs)
    };
    ...
    assert!(
        word.what.contains("invalid type") && word.what.contains("thirty"),
        "a string is not a count: {word}"
    );

    // an allowance of zero would serve nobody
    let allowance = |r: &str, s: &str| {
        Config::parse(
            &with("requests = 120", &format!("requests = {r}"))
                .replace("seconds = 60", &format!("seconds = {s}")),
        )
    };
    assert!(
        allowance("0", "60")
    ...
    assert_eq!(
        ing("verified-on-acceptance").unwrap().ingestion,
        Ingestion::VerifiedOnAcceptance
    );
    ...
    let limits = |m: &str, f: &str| {
        Config::parse(&format!(
            "{GOOD}\n[resource-limits]\nmemory = {m}\nfuel = {f}\n"
        ))
    };
    assert_eq!(
        limits("1024", "5").unwrap().resource_limits,
        Some((1024, 5))
    );
    assert!(
        limits("0", "5")
...
fn an_upstream_names_a_keyhash_and_the_addresses_it_is_reached_at() {
    ...
    assert_eq!(
        addrs,
        vec![
            "10.0.0.1:7431".parse().unwrap(),
            "10.0.0.2:7431".parse().unwrap()
        ]
    );
    ...
    assert!(
        bad(&format!("node = \"{k}\"\n")).what.contains("addresses"),
        "and the field it wanted is named"
    );
}
...
fn the_reconciliation_interval_defaults_and_bounds_and_can_be_turned_off() {
    let every = |v: &str| {
        ...
        .map(|c| c.reconcile_secs)
    };
    assert_eq!(
        Config::parse(&with("heartbeat = 30", "heartbeat = 30"))
            .unwrap()
            .reconcile_secs,
        900,
        "a default, not a rule any document states"
    );
    ...
    assert_eq!(
        every("0").unwrap(),
        0,
        "zero turns the replay off rather than being refused"
    );
    ...
    assert!(
        word.what.contains("invalid type"),
        "a string is not a count: {word}"
    );
}
