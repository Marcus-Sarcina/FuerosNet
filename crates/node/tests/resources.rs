...
fn register(
    ...
    .encode();
    let reply = svc
        .register(&ids(), &kh(peer), &body)
        .expect("a well-framed registration is answered");
...
fn query(
    ...
    let q = CatalogQuery {
        service_type: filter.map(|s| s.to_string()),
        nonce: [9; 16],
    }
    .encode();
    let reply = svc
        .answer(&kh(asker), &q, scopes)
        .map(|b| CatalogReply::decode(&b).unwrap());
    if let Some(r) = &reply {
...
fn a_registration_is_accepted_only_over_the_owners_own_session() {
    ...
    assert_eq!(
        register(&mut svc, "carol", &e, None, 1),
        REGISTRATION_REFUSED,
        "P's session, O's entry"
    );
    assert!(svc.held(&R1).is_none());
    assert_eq!(
        register(&mut svc, "bob", &e, None, 2),
    ...
    assert_eq!(
        register(&mut svc, "carol", &e, None, 3),
        REGISTRATION_REFUSED
    );
    assert_eq!(svc.held(&R1).unwrap().bytes, e);
    let _ = &sc.table;
...
fn a_keyhash_served_under_one_owner_is_refused_to_another() {
    ...
    assert_eq!(
        register(&mut svc, "bob", &e1, None, 1),
        REGISTRATION_RECORDED
    );
    let e2 = entry("carol", R1, "rhtn-forum", b"two");
    assert_eq!(
        register(&mut svc, "carol", &e2, None, 2),
        REGISTRATION_REFUSED
    );
    assert_eq!(
        svc.held(&R1).unwrap().owner,
        kh("bob"),
        "H still serves O1's entry"
    );
...
fn re_registration_replaces_and_nothing_superseded_is_retained_or_archived() {
    ...
    assert_eq!(
        register(&mut svc, "bob", &e1, Some(Scope::Dunbar), 1),
        REGISTRATION_RECORDED
    );
    assert_eq!(
        register(&mut svc, "bob", &e2, None, 2),
        REGISTRATION_RECORDED
    );
    ...
    assert!(
        !format!("{svc:?}").contains(&format!("{:?}", e1)),
        "E1 is absent from H's storage"
    );
    assert_eq!(
        view.archive.len(),
        archive_len,
        "the archive did not advance"
    );
    view.catalog = svc;
...
fn a_first_registration_defaults_to_scope_self() {
    ...
    assert_eq!(
        query(&svc, &scopes, "bob", None).unwrap().entries,
        vec![e.clone()],
        "O sees its own entry"
    );
    assert!(
        query(&svc, &scopes, "carol", None)
            .unwrap()
            .entries
            .is_empty(),
        "a horizon member does not"
    );
}
...
fn withdrawal_is_re_registration_with_scope_self_and_nothing_propagates() {
    ...
    assert_eq!(
        register(&mut view.catalog, "bob", &e, Some(Scope::Dunbar), 1),
        REGISTRATION_RECORDED
    );
    assert_eq!(
        query(&view.catalog, &scopes, "carol", None)
            .unwrap()
            .entries
            .len(),
        1,
    ...
    assert_eq!(
        register(&mut view.catalog, "bob", &e, Some(Scope::Own), 2),
        REGISTRATION_RECORDED
    );
    assert!(
        query(&view.catalog, &scopes, "carol", None)
    ...
    assert!(
        fab.frames().is_empty(),
        "no TopologyPush or other frame carrying the entry leaves H"
    );
...
fn an_asker_outside_the_horizon_gets_no_reply_at_all() {
    ...
    assert_eq!(
        query(&svc, &scopes, "w2", None),
        None,
        "the stream is closed: no CatalogReply, empty or otherwise"
    );
...
fn entries_come_ordered_by_resource_then_owner_111_at_a_time_with_the_next_type_named() {
    ...
    let mut resources: Vec<[u8; 32]> = (0..113u32)
        .map(|i| rhtn_codec::cose::sha256(&i.to_be_bytes()))
        .collect();
    resources.sort();
    for (i, r) in resources.iter().enumerate() {
        ...
        assert_eq!(
            register(&mut svc, "bob", &e, Some(Scope::Dunbar), (i % 250) as u8),
            REGISTRATION_RECORDED
        );
    }
    let scopes = TableScopes {
    ...
    let got: Vec<[u8; 32]> = reply
        .entries
        .iter()
        .map(|b| CatalogEntry::parse(b).unwrap().resource)
        .collect();
    assert_eq!(
        got,
        resources[..111].to_vec(),
        "ascending resource-then-owner keyhash order"
    );
    assert_eq!(
        reply.continuation.as_deref(),
        Some("rhtn-wiki"),
        "the type of the 112th"
    );
...
fn the_owners_signature_is_returned_unchanged_and_none_is_added() {
    ...
    for asker in ["carol", "w1"] {
        ...
        assert!(
            !reply.encode().windows(32).any(|w| w == kh("alice")),
            "nothing by H"
        );
    }
}
...
fn an_entry_whose_scope_cannot_be_computed_is_stored_and_grants_nothing() {
    ...
    let e = CatalogEntry::build(
        ...
        &EntryFields {
            ...
            instance: "x".into(),
            endpoint: b"e".to_vec(),
            connect_scope: Some(Scope::List(vec![far])),
            metadata: None,
            data_practice: None,
        },
    );
    ...
    .encode();
    assert_eq!(
        g.serve(&kh("alice"), &sc.table, &kh("carol"), &req).status,
        STATUS_NO_ROLE
    );
    g.set_row(
        R1,
...
fn an_unknown_data_practice_is_retained_and_served() {
    ...
    let reply = query(&svc, &scopes, "carol", None).unwrap();
    assert_eq!(
        CatalogEntry::parse(&reply.entries[0])
            .unwrap()
            .data_practice,
        Some(7),
        "field 9 unchanged"
...
fn a_request_is_evaluated_in_the_normative_order_and_answered_with_the_one_code_its_step_yields() {
    ...
    // a stranger
    assert_eq!(
        g.serve(&me, &sc.table, &kh("w2"), &request(R1, OK_GET))
            .status,
        STATUS_REFUSED
    );
    // a member lacking a SubtreeAck: w1, adopted under carol
    assert_eq!(
        g.serve(&me, &sc.table, &kh("w1"), &request(R1, OK_GET))
            .status,
        STATUS_NO_ACK
    );
    ...
    assert_eq!(
        g.serve(&me, &sc.table, &kh("carol"), &request(R1, OK_GET))
            .status,
        STATUS_UNAVAILABLE
    );
    assert_eq!(
    ...
    assert_eq!(
        backend.calls.load(Ordering::SeqCst),
        1,
        "only the last reached the backend"
    );
    // step 0: a body that does not decode
    assert_eq!(
        g.serve(&me, &sc.table, &kh("carol"), b"\xff\xff").status,
        STATUS_MALFORMED
    );
}
...
fn a_non_member_is_refused_for_everything_existing_or_not() {
    ...
    let b = g.serve(
        ...
        &sc.table,
        &kh("w2"),
        &request(unknown, OK_GET),
    );
    assert_eq!(
        (a.clone(), b.clone()),
        (
            ResourceResponse::code(STATUS_REFUSED),
            ResourceResponse::code(STATUS_REFUSED)
        )
    );
...
fn a_request_is_decided_from_the_materialised_row_and_a_row_wider_than_64_is_refused() {
    ...
    assert_eq!(
        ...
        "delivered on the strength of the row"
    );
    ...
    assert_eq!(
        ...
        Err(RowError::Undeclared("admin".into()))
    );
}
...
fn caller_rhtn_headers_are_stripped_the_nodes_inserted_and_a_session_minted_per_resource() {
    ...
    assert_eq!(
        g.serve(&kh("alice"), &sc.table, &kh("carol"), &request(R1, forged))
            .status,
        STATUS_DELIVERED
    );
    assert_eq!(
        g.serve(&kh("alice"), &sc.table, &kh("carol"), &request(R2, forged))
            .status,
        STATUS_DELIVERED
    );
    ...
    for (h, res, authority) in [(&h1, R1, "r1.internal"), (&h2, R2, "r2.internal")] {
        assert_eq!(
            get(h, "rhtn-principal"),
            vec![http::base64url(&http::pairwise_principal(
                &res,
                &kh("carol")
            ))]
        );
        ...
        assert_eq!(
            ...
            "routed by the resource, not the caller's host"
        );
        assert_eq!(get(h, "x-app"), vec!["keep".to_string()]);
        assert!(
            !h.iter().any(|(_, v)| v == "admin" || v == "somebody"),
            "the forged values are absent"
        );
    }
    assert_ne!(
        get(&h1, "rhtn-session"),
        get(&h2, "rhtn-session"),
        "one identifier per resource"
    );
    assert_ne!(
        get(&h1, "rhtn-principal"),
        get(&h2, "rhtn-principal"),
        "pairwise per resource"
    );
    // the same session identifier on the next request to the same resource
    g.serve(&kh("alice"), &sc.table, &kh("carol"), &request(R1, OK_GET));
    assert_eq!(
        get(
            &http::headers_of(&b1.seen.lock().unwrap()[1]),
            "rhtn-session"
        ),
        get(&h1, "rhtn-session")
    );
...
fn ambiguous_messages_are_rejected_and_exactly_one_message_is_emitted() {
    ...
    for c in cases {
        assert_eq!(
            g.serve(&kh("alice"), &sc.table, &kh("carol"), &request(R1, c))
                .status,
            STATUS_MALFORMED,
            "{:?}",
    ...
    let chunked =
        b"POST /p HTTP/1.1\r\nhost: x\r\ntransfer-encoding: chunked\r\n\r\n3\r\nabc\r\n0\r\n\r\n";
    assert_eq!(
        g.serve(&kh("alice"), &sc.table, &kh("carol"), &request(R1, chunked))
            .status,
        STATUS_DELIVERED
    );
...
fn a_role_change_ends_the_hosted_session_and_leaves_the_transport_alone() {
    ...
    .unwrap();
    assert_eq!(
        g.session(&kh("carol"), &R1),
        None,
        "the resource-facing session is dropped"
    );
    assert_eq!(
        g.serve(&kh("alice"), &sc.table, &kh("carol"), &request(R1, OK_GET))
            .status,
...
fn a_subtree_is_acknowledged_under_policy_without_a_prompt_and_lapses_with_the_relationship() {
    ...
    .unwrap();
    assert_eq!(
        g.serve(&kh("alice"), &sc.table, &kh("w1"), &request(R1, OK_GET))
            .status,
        STATUS_DELIVERED
    );
    ...
    assert_eq!(
        g.serve(&kh("alice"), &sc.table, &kh("w1"), &request(R1, OK_GET))
            .status,
        STATUS_REFUSED,
        "G is no longer in H's Dunbar Org at all"
    );
}
...
fn an_abuse_report_is_stored_only_from_the_resource_it_names_and_carried_nowhere() {
    ...
    assert_eq!(
        register(&mut view.catalog, "bob", &e, None, 1),
        REGISTRATION_RECORDED
    );
    let genuine = AbuseReport::build(
        &id("w4"),
        1_800_000_000,
        ABUSE_EXCESSIVE_LOAD,
        Some(b"burst"),
    );
    let mut forged = AbuseReport::build(
        &id("w5"),
        1_800_000_000,
        ABUSE_EXCESSIVE_LOAD,
        Some(b"burst"),
    ...
    assert_eq!(
        view.catalog.take_report(&ids(), &genuine),
        Ok(kh("bob")),
        "stored for O"
    );
    assert!(
        view.catalog.take_report(&ids(), &forged).is_err(),
        "rejected"
    );
...
fn the_catalog_page_shows_what_the_viewer_holds_with_the_roles_held() {
    ...
    assert_eq!(
        page,
        vec![(e1.clone(), vec!["reader".to_string()])],
        "R2, with no connect, is not listed"
    );
}
...
fn a_header_value_carrying_a_control_character_is_refused_and_writes_no_header_of_its_own() {
    ...
    assert_eq!(
        g.serve(&kh("alice"), &sc.table, &kh("carol"), &request(R1, ok))
            .status,
        STATUS_DELIVERED
    );
    let seen = backend.seen.lock().unwrap()[0].clone();
    let roles: Vec<String> = http::headers_of(&seen)
        .into_iter()
        .filter(|(k, _)| k == "rhtn-roles")
        .map(|(_, v)| v)
        .collect();
    assert_eq!(
        roles,
        vec![String::new()],
        "one roles header, the node's, and this caller holds none"
    );
}
...
fn a_chunk_length_larger_than_the_message_is_refused_rather_than_computed_past_it() {
    ...
    let over =
        b"POST /p HTTP/1.1\r\nhost: x\r\ntransfer-encoding: chunked\r\n\r\nff\r\nabc\r\n0\r\n\r\n";
    assert_eq!(
        g.serve(&kh("alice"), &sc.table, &kh("carol"), &request(R1, over))
            .status,
        STATUS_MALFORMED
    );
    assert_eq!(
...
fn a_directional_scope_grants_nothing_outside_the_owners_dunbar_org() {
    ...
    assert!(
        table.horizon(&kh("bob"), 2).contains(&kh("w5")),
        "the asker is inside the host's own horizon"
    );
    let mut svc = CatalogService::default();
    let down = entry("alice", R1, "rhtn-forum", b"down");
    assert_eq!(
        register(&mut svc, "alice", &down, Some(Scope::Down(3)), 1),
        REGISTRATION_RECORDED
    );
    assert!(
        query(&svc, &scopes, "w5", None).unwrap().entries.is_empty(),
        "three edges down is outside alice's Dunbar Org"
    );
    // the reverse: w5 owning, alice three edges up
    let mut up = CatalogService::default();
    let e = entry("w5", R2, "rhtn-forum", b"up");
    assert_eq!(
        register(&mut up, "w5", &e, Some(Scope::Up(3)), 2),
        REGISTRATION_RECORDED
    );
    assert!(
        query(&up, &scopes, "alice", None)
            .unwrap()
            .entries
            .is_empty(),
        "three edges up is outside w5's Dunbar Org"
    );
    ...
    assert_eq!(
        query(&svc, &scopes, "bob", None).unwrap().entries,
        vec![down.clone()],
        "one edge down is inside it"
    );
    assert_eq!(
        query(&svc, &scopes, "w4", None).unwrap().entries,
        vec![down.clone()],
        "two edges down is inside it"
    );
}
...
fn a_reserved_role_name_is_refused_where_it_is_written_and_never_reaches_a_backend() {
    ...
    let all: BTreeSet<String> = ["connect", "discover", "read"]
        ...
        .collect();
    assert!(
        matches!(g.set_row(R2, kh("carol"), Row { roles: all, connect: true }), Err(RowError::Reserved(r)) if r == "connect")
    );
    ...
    assert_eq!(
        g.serve(&kh("alice"), &sc.table, &kh("carol"), &request(R2, msg))
            .status,
        STATUS_DELIVERED
    );
    let seen = backend.seen.lock().unwrap()[0].clone();
    ...
    assert_eq!(
        roles,
        vec!["read".to_string()],
        "the application role, and neither reserved name"
    );
}
...
fn a_standing_grant_follows_the_owners_horizon_in_and_out() {
    ...
    for inside in ["alice", "bob", "carol", "w1"] {
        assert_eq!(
            g.row(&R1, &kh(inside)).map(|r| r.connect),
            Some(true),
            "{inside} is in the owner's horizon"
        );
    }
    assert!(
        g.row(&R1, &kh("w2")).is_none(),
        "w2 hangs off w3 and is outside it"
    );
    ...
    assert!(
        g.row(&R1, &kh("carol")).is_none(),
        "the departing party's row goes"
    );
    assert_eq!(
        g.row(&R1, &kh("bob")).map(|r| r.connect),
        Some(false),
        "and the one still inside is untouched"
    );
}
...
fn an_ending_purges_the_departed_subtree_from_every_resource_not_only_the_granted_ones() {
    ...
    for gone in ["carol", "w1"] {
        assert!(
            g.row(&R1, &kh(gone)).is_none(),
            "{gone} is off the granted resource"
        );
        assert!(
            g.row(&R2, &kh(gone)).is_none(),
            "and off the hand-written one"
        );
        assert!(
            g.session(&kh(gone), &R1).is_none(),
            "with no session left open"
        );
    }
    assert!(
        g.row(&R1, &kh("bob")).is_some(),
        "the party still inside keeps its rows"
    );
...
fn replacing_a_standing_grant_rewrites_what_it_wrote_and_leaves_the_operators_own_rows() {
    ...
    g.set_row(R1, kh("bob"), pinned.clone())
        .expect("the operator's own row");
    ...
    assert_eq!(
        g.row(&R1, &kh("bob")),
        Some(&pinned),
        "and the operator's own row is untouched"
    );
    assert_eq!(
        g.row(&R2, &kh("carol")).map(|r| r.connect),
        Some(true),
    ...
    assert_eq!(
        g.row(&R1, &kh("carol")),
        Some(&open),
        "the grant's rows follow it back out"
    );
    assert_eq!(
        g.row(&R1, &kh("bob")),
        Some(&pinned),
