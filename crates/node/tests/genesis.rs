...
use rhtn_node::resolution::{
    AnchorEntry, AnchorTable, Carried, Ingestion, Path, REQUEST_RESOLVE, ResolveReply,
    ResolveRequest, Step, anchor_entry,
};
...
fn subnet() -> (World, NodeView, NodeView, NodeView) {
    ...
    let mut alice = view(
        "alice",
        table_with(kh("alice"), &w, &[&a_bob, &a_carol], &infra),
        "alice",
        &[],
    );
    let mut bob = view(
        "bob",
        table_with(kh("bob"), &w, &[&a_bob, &a_carol], &infra),
        "alice",
        &[1],
    );
    let mut carol = view(
        "carol",
        table_with(kh("carol"), &w, &[&a_bob, &a_carol], &infra),
        "alice",
        &[1, 2],
    );
...
fn a_new_root_comes_into_being_needing_nothing() {
    ...
    assert_eq!(
        r.archive.next_back_pointers(),
        vec![rhtn_archive::genesis(&me)],
        "its chain begins at genesis"
    );
    assert_eq!(
        r.evidence().known(),
        BTreeSet::from([me]),
    ...
    let fab = Fabric::with(&[]);
    assert_eq!(
        r.require_currency(&*fab, &ids(), &me, None, None),
...
fn a_new_root_s_peering_propagates_through_its_peer_into_an_existing_subnet() {
    ...
    let body = r
        ...
        .expect("a naked root peers");
    let peering = Record::parse(&tx::envelope(
        tx::TYPE_PEERING,
        &body,
        &[&id("w9"), &id("bob")],
    ))
    ...
    assert_eq!(
        r.originate_push(&*rfab, KIND_TRANSACTION, &peering.bytes, &ids()),
        Decision::Stored
    );
    assert_eq!(
        rfab.recipients(FRAME_TOPOLOGY_PUSH),
        BTreeSet::from([kh("bob")])
    );
    ...
    assert_eq!(
        bob.take_object(&*bfab, &kh("w9"), KIND_TRANSACTION, &peering.bytes, &ids()),
        Decision::Stored
    );
    assert_eq!(
        bfab.recipients(FRAME_TOPOLOGY_PUSH),
        BTreeSet::from([kh("alice"), kh("carol")])
    );
    ...
    assert_eq!(
        alice.take_object(&*afab, &kh("bob"), KIND_TRANSACTION, &peering.bytes, &ids()),
        Decision::Stored
    );
    assert_eq!(
        carol.take_object(&*cfab, &kh("bob"), KIND_TRANSACTION, &peering.bytes, &ids()),
        Decision::Stored
    );
    assert_eq!(
        afab.count(FRAME_TOPOLOGY_PUSH) + cfab.count(FRAME_TOPOLOGY_PUSH),
        0,
        "nothing goes back the way it came"
    );
    assert!(
        carol.peers_of(&kh("bob")).contains(&kh("w9")),
        "a node two hops from R holds R's edge"
    );
    // R's address travels in the peering record itself
    let held = carol
        .peerings()
        .into_iter()
        .find(|p| p.a == kh("w9") || p.b == kh("w9"))
        .expect("held");
    ...
    assert_eq!(
        (alice.standing(&kh("w9")), carol.standing(&kh("w9"))),
        (10.0, 10.0)
    );
    ...
    assert_eq!(
        bob.take_object(&*bfab, &kh("w9"), KIND_ENDPOINT_RECORD, &er, &ids()),
        Decision::OutOfStore
    );
}
...
fn an_existing_node_reaches_a_new_root_through_its_anchor_table() {
    ...
    let entry = AnchorEntry::parse(&anchor_entry(
        &id("w9"),
        &[point(9, 7009)],
        0,
        Seqno {
    ...
    let (mut res, carried) = bob
        .resolve(&*bfab, &anchors, kh("w9"), kh("w9"), Path::empty(), [3; 16])
        .unwrap();
    assert!(matches!(
        carried,
        Carried::Direct {
            sent_on_session: true
        }
    ));
    ...
    assert_eq!(
        (req.subject, req.anchor, req.nibbles),
        (kh("w9"), kh("w9"), 0)
    );
    ...
    assert!(
        bob.resolve(&*bfab, &empty, kh("w9"), kh("w9"), Path::empty(), [4; 16])
            .is_err()
    );
    assert_eq!(
        bfab.requests_to(&kh("w9"), REQUEST_RESOLVE).len(),
        1,
        "no second request left"
    );
}
