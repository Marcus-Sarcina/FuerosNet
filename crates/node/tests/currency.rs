...
fn a_patron_issues_a_fresh_attestation_for_its_subordinate() {
    ...
    let CurrencyReply::Attestation { nonce: n, bytes } = &reply else {
        panic!("{reply:?}")
    };
    assert_eq!(*n, nonce(1));
    let a = att(bytes);
...
fn a_refresh_is_a_fresh_issuance_never_an_extension() {
    ...
    assert_ne!(
        (a1.subject, a1.current, a1.issued_at),
        (a0.subject, a0.current, a0.issued_at)
    );
}
...
fn recovery(old: &str, new: &str, patron: &str, verifier: &str) -> Record {
    ...
    let a = tx::Adoption {
        ...
        presented_head: None,
        back: [
            &[rhtn_archive::genesis(&n.public.keyhash)],
            &[rhtn_archive::genesis(&p.public.keyhash)],
        ],
    };
    Record::parse(&tx::envelope(
        tx::TYPE_ADOPTION,
        &tx::adoption_body(&a),
        &[&n, &p],
    ))
...
fn nothing_is_issued_for_a_key_the_issuer_has_superseded() {
    ...
    p.table
        .apply(&rec, &ids(), &no_presence(), None)
        .expect("validated");
    let k2 = kh("carol");
    assert!(p.is_superseded(&k1));
    assert_eq!(
        p.table.current_key(&k1),
        k2,
        "the recovery overwrote P's record of the subject"
    );
    ...
    for subject in [k1, k2] {
        ...
        let CurrencyReply::Attestation { bytes, .. } = &reply else {
            panic!("{reply:?}")
        };
        let a = att(bytes);
...
fn an_adoption_is_countersigned_whatever_the_subjects_staple_says() {
    ...
    let body = n
        .propose_adoption(
            &x,
            Evidence::Presence(pop.txid),
            5,
            &[rhtn_archive::genesis(&x)],
        )
        .expect("produced with no staple");
    ...
    let stale = tx::currency_attestation(
        &id("carol"),
        &x,
        &x,
        n.now() - 40_000,
    ...
    let body = n
        ...
        .expect("produced with an expired staple");
    let rec = n.countersign_adoption(&body, &id("bob"), &ids()).unwrap();
...
fn routine_payload_fails_open_on_an_expired_staple() {
    ...
    assert_eq!(
        n.take_staple(&ids(), &kh("bob"), &stale, &[]),
        Staple::Expired
    );
    assert_eq!(n.staple_for(&ids(), &kh("bob"), &[]), Staple::Expired);
    assert_eq!(
        gate(n.is_superseded(&kh("bob"))),
        Gate::Proceed,
        "delivery proceeds"
    );
...
fn an_unexpired_staple_does_not_restore_a_superseded_binding() {
    ...
    let staple = tx::currency_attestation(
        &id("alice"),
        &k1,
        &k1,
        n.now(),
    ...
    assert_eq!(
        state,
        Staple::Current,
        "unexpired and validly signed by S's patron"
    );
    ...
    assert!(
        matches!(
            n.require_currency(&*fab, &ids(), &k1, None, None),
            Requirement::Settled(Gate::Refuse(_))
        ),
        "no session is served under k1"
    );
    assert!(
        matches!(gate(n.is_superseded(&k1)), Gate::Refuse(_)),
        "nothing is delivered under k1"
    );
}
...
fn a_sibling_issues_secondhand_while_the_patron_is_dark() {
    ...
    assert_eq!(
        p2.rung_for(&cur, &kh("carol")),
        None,
        "not while the patron answers"
    );
    ...
    assert_eq!(
        p2.rung_for(&cur, &kh("carol")),
        None,
        "the staple still stands"
    );
    // dark for the lifetime: the sibling rung opens
    cur.unreachable
        .insert(kh("bob"), p2.now() - cur.sibling_after);
    ...
    assert_eq!(
        relying.read_staple(&ids(), &a.bytes, &kh("carol"), &[]).1,
        Staple::Current
    );
}
...
fn the_grandpatron_issues_once_the_patron_and_its_siblings_are_dark() {
    ...
    cur.unreachable
        .insert(kh("bob"), g.now() - cur.grandpatron_after);
    ...
    assert_eq!(
        g.rung_for(&cur, &kh("carol")),
        None,
        "not until every sibling has been dark for days"
    );
    cur.unreachable
        .insert(kh("w1"), g.now() - cur.grandpatron_after);
    ...
    assert_eq!(
        ladder_view("w1")
            .read_staple(&ids(), &a.bytes, &kh("carol"), &[])
            .1,
        Staple::Current
    );
}
...
fn a_peering_is_proposed_with_no_staple_however_many_issuers_are_dark() {
    ...
    for issuer in ["bob", "w1", "alice"] {
        ...
        assert!(
            v.rung_for(&cur, &kh("carol")).is_none() || cur.unreachable.contains_key(&kh(issuer))
        );
    }
    let stale = tx::currency_attestation(
        &id("bob"),
    ...
    assert_eq!(
        s.take_staple(&ids(), &kh("carol"), &stale, &[]),
        Staple::Expired
    );
    ...
    let first = s
        .propose_peering(&kh("w5"), &other_point, &pop, &other_back)
        .expect("the peering transaction is produced");
    ...
    assert_eq!(
        fab.request_count(REQUEST_CURRENCY),
        0,
        "nothing about S's currency was asked of anyone"
    );
}
...
fn a_caller_with_a_stale_staple_asks_its_introducer_first() {
    ...
    assert_eq!(
        ask.asked,
        vec![introducer],
        "N's first request goes toward I"
    );
    assert_eq!(fab.requests_to(&introducer, REQUEST_CURRENCY).len(), 1);
    assert_eq!(
        fab.requests_to(&patron, REQUEST_CURRENCY).len(),
        0,
        "and none toward X's patron"
    );
    ...
    assert_eq!(
        n.on_currency_reply(&*fab, &ids(), &mut ask, &cannot),
        AskStep::Exhausted
    );
    ...
    let ask2 = n
        .ask_currency(&*fab, x, Some(introducer), Some(patron), nonce(11))
        .unwrap();
...
fn answering_a_currency_request_retains_nothing() {
    ...
    assert_eq!(
        before, after,
        "no record of the request, of Q, or of the reply"
    );
    ...
    assert_eq!(
        encoded.len(),
        1 + (1 + 2 + 32) + (1 + 1 + 16),
        "subject and nonce, and nothing else"
    );
}
...
fn expiry_is_decided_against_the_relying_partys_own_clock() {
    ...
    n.set_now(t - 1);
    assert_eq!(
        n.read_staple(&ids(), &staple, &kh("bob"), &[]).1,
        Staple::Current
    );
    n.take_staple(&ids(), &kh("bob"), &staple, &[]);
    assert_eq!(
        n.require_currency(&*fab, &ids(), &kh("bob"), Some(kh("w1")), None),
        Requirement::Settled(Gate::Proceed),
        "current: nothing to ask"
    );
    assert_eq!(fab.request_count(REQUEST_CURRENCY), 0);
    n.set_now(t + 1);
    assert_eq!(
    ...
    assert!(
        ...
        "expired: the fallback query is sent"
    );
...
fn a_staple_from_an_issuer_the_relying_party_cannot_place_is_not_current() {
    ...
    let forged =
        tx::currency_attestation(&id("w9"), &subject, &subject, t, t + 36_000, ROLE_PATRON);
    assert_eq!(
        relying.read_staple(&ids(), &forged, &subject, &[]).1,
        Staple::WrongIssuer
    );
    // a real party on the wrong rung
    let wrong_rung =
        tx::currency_attestation(&id("w1"), &subject, &subject, t, t + 36_000, ROLE_PATRON);
    assert_eq!(
        relying.read_staple(&ids(), &wrong_rung, &subject, &[]).1,
        Staple::WrongIssuer,
        "P' is a sibling, not the patron"
    );
    let right_rung =
        tx::currency_attestation(&id("w1"), &subject, &subject, t, t + 36_000, ROLE_SIBLING);
    assert_eq!(
        relying.read_staple(&ids(), &right_rung, &subject, &[]).1,
        Staple::Current
    );
    ...
    let _ = w.tick();
    assert_eq!(
        blank.read_staple(&ids(), &right_rung, &subject, &[]).1,
        Staple::UnknownIssuer
    );
    // unless the introduction named the patron
    let by_patron =
        tx::currency_attestation(&id("bob"), &subject, &subject, t, t + 36_000, ROLE_PATRON);
    assert_eq!(
        blank
            .read_staple(&ids(), &by_patron, &subject, &[kh("bob")])
            .1,
        Staple::Current
    );
}
...
fn issuance_and_the_ladder_read_a_clock_that_moves() {
    ...
    let mut g = view(
        "alice",
        table_with(kh("alice"), &w, &[&a_n, &a_s], &["alice", "bob"]),
        "alice",
        &[],
    );
    ...
    let a = rhtn_archive::currency::parse_attestation(
        &ids(),
        &g.issue_currency(&cur, &kh("bob")).unwrap(),
    )
    .unwrap();
    ...
    assert_eq!(
        a.issued_at,
        t0 + 1,
        "the next issuance reads the clock again"
    );
    ...
    assert_eq!(
        g.rung_for(&cur, &kh("carol")),
        Some(Rung::Grandpatron),
        "the interval elapsed on the clock alone"
    );
}
...
fn a_currency_ask_goes_on_a_request_stream_and_its_reply_settles_it() {
    ...
    let ask = n
        .ask_currency(&*fab, x, None, Some(patron), nonce(18))
        .expect("asked");
    assert_eq!(
        fab.requests_to(&patron, REQUEST_CURRENCY).len(),
        1,
        "one request stream toward the patron"
    );
    ...
    let att =
        tx::currency_attestation(&id("carol"), &x, &x, n.now(), n.now() + 36_000, ROLE_PATRON);
    let reply = CurrencyReply::Attestation {
        nonce: ask.nonce,
        bytes: att,
    }
    .encode();
    assert_eq!(
        n.take_currency_reply(&*fab, &ids(), &reply),
        Some(AskStep::Current)
    );
    assert!(!n.asks.contains_key(&ask.nonce));
    assert!(
        n.staples.contains_key(&x),
        "the attestation is the staple now held"
    );
    ...
    below.serve(&patron);
    assert!(
        n.ask_currency(&*below, x, None, Some(patron), nonce(19))
            .is_none()
    );
...
fn a_valid_attestation_whose_current_key_differs_from_its_subject_is_accepted() {
    ...
    assert_eq!(
        state,
        Staple::Current,
        "field 2 differing from field 1 is not a ground for rejection"
    );
    assert_eq!(
        parsed.unwrap().current,
        kh("w9"),
        "field 2 really does name a different key"
    );
...
fn a_currency_attestation_does_not_redirect_the_addressed_key() {
    ...
    assert_eq!(
        att(&staple).current,
        kh("w9"),
        "the staple did name a different key"
    );
}
