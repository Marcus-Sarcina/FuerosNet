...
| `ffi/` | `rhtn-ffi`: the one boundary the mobile shells bind to — the client's operations outward, the platform's camera, channels and clock inward, and the value types that cross; a device holding a delegation and no seed started from the identity's material, its transport seed and the run its ceremony device signed, with delegation issuance and the bundle's signature crossing the boundary in either direction; the client's state written through the platform's storage object and read back whole at the next start, a backup exported and restored; the sibling cache kept across sessions and restarts, the serving node dialled first and the cached siblings where it is dark, failover after three missed intervals driven inside the kernel and shown as a status and an event, and maintenance on the kernel's own clock; the direct path on a socket of the kernel's own, gated to the horizon and to the person's override in both directions, and the catalog swept and shown; the whole surface annotated for `uniffi`, with a `harness` feature exporting a test node and identities for a binding's own round trip and `kotlin/RoundTrip.kt` driving the generated Kotlin against a real node. No decision is taken at the boundary that is not taken below it |
