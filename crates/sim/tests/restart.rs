...
async fn a_recovery_the_node_applies_ends_the_old_credentials_service() {
    ...
    let AttachOutcome::Attached(_alice) =
        attach(&acfg, &client_ep(), kh("bob"), n.addr, false).await
    else {
        panic!("alice attaches")
    };
    assert!(
        until(2000, || n.node.has_session(&kh("alice"))).await,
        "served"
    );
    ...
    assert!(
        n.view.lock().unwrap().is_superseded(&kh("alice")),
        "the table knows"
    );
    assert!(
        n.node.is_superseded(&kh("alice")),
        "so does the serving state, with no separate call"
    );
    assert!(
        until(3000, || !n.node.has_session(&kh("alice"))).await,
        "alice's session ended"
    );
    assert!(
        matches!(
            attach(&acfg, &client_ep(), kh("bob"), n.addr, false).await,
        ...
        "a fresh attach as alice is refused"
    );
    assert_eq!(
        n.node.enqueue(kh("alice"), b"late".to_vec()),
        Err(rhtn_transport::queue::Refusal::Superseded)
    );
    ...
    assert!(
        n2.node.is_superseded(&kh("alice")),
        "seeded from the store at start"
    );
    assert!(matches!(
        attach(&acfg, &client_ep(), kh("bob"), n2.addr, false).await,
        AttachOutcome::Refused
    ));
    let _ = std::fs::remove_dir_all(&dir);
}
...
async fn a_restarted_node_advances_its_endpoint_counter_only_for_a_new_address() {
    ...
    assert_eq!(
        n1.view.lock().unwrap().position.seqno.counter,
        1,
        "the position advanced with it"
    );
    ...
    let second = n2
        ...
        .clone();
    assert_eq!(
        (second.seqno.series, second.seqno.counter),
        (1, 2),
        "a changed address advances the counter"
    );
    assert_eq!(n2.view.lock().unwrap().position.seqno.counter, 2);
    let own: Vec<std::net::SocketAddr> = n2
        .view
        .lock()
        .unwrap()
        .own_endpoints()
    ...
    assert_eq!(
        own,
        vec![n2.addr],
        "the current address is what the store serves"
    );
    ...
    let again = n2
        ...
        .lock()
        .unwrap()
        .publish_own_endpoints(&kh("alice"), &[point])
        .unwrap();
...
async fn a_running_node_asks_currency_up_its_session_and_takes_the_answer() {
    ...
    let AttachOutcome::Attached(_up) = n.attach_upstream(&ncli, kh("alice")).await else {
        panic!("N attaches to P")
    };
    ...
    let ask = n
        ...
        .lock()
        .unwrap()
        .ask_currency(&n.adjacency, kh("carol"), None, Some(kh("alice")), [8; 16])
        .expect("asked up the session");
    ...
    assert!(
        ...
        .await,
        "P's attestation came back on the request stream and is N's staple"
    );
    ...
    assert!(
        p.view
            .lock()
            .unwrap()
            .ask_currency(&p.adjacency, kh("carol"), Some(kh("bob")), None, [9; 16])
            .is_none()
    );
}
