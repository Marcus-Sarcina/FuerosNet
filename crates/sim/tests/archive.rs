...
async fn a_node_serves_its_own_archive_on_request_and_an_empty_batch_for_another_subject() {
    ...
    let mut n_view = view_of(
        "bob",
        table_of("bob", &s, &records, &["alice", "bob"]),
        "alice",
        &[0],
        s.clock,
    );
    ...
    let AttachOutcome::Attached(c) = attach(&ccfg, &client_ep(), kh("bob"), n.addr, false).await
    else {
    ...
    let reply = ArchiveReply::decode(
        &c.request(REQUEST_ARCHIVE, &req.encode())
            .await
            .expect("answered"),
    )
    .unwrap();
    ...
    let reply = ArchiveReply::decode(
        &c.request(REQUEST_ARCHIVE, &other.encode())
            .await
            .expect("answered"),
    )
    .unwrap();
