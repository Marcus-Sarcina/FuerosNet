...
use rhtn_node::resolution::{
    AnchorEntry, AnchorTable, Ingestion, Path, REQUEST_CURRENCY, REQUEST_RESOLVE, ResolveReply,
    ResolveRequest, anchor_entry,
};
...
async fn a_new_root_peers_floods_and_is_reached_over_real_sessions() {
    ...
    let AttachOutcome::Attached(upstream) = n.attach_upstream(&ncli, kh("alice")).await else {
        panic!("N attaches to P")
    };
    let ccfg = client_cfg("carol");
    know(&ccfg, "bob", n.addr);
    let AttachOutcome::Attached(mut c) =
        attach(&ccfg, &client_ep(), kh("bob"), n.addr, false).await
    else {
        panic!("C attaches to N")
    };
    let mut cv = view_of(
        "carol",
    ...
    assert!(
        !r.view.lock().unwrap().own_endpoints().is_empty(),
        "it published its own endpoint record at start"
    );
    ...
    let body = r
        ...
        .propose_peering(
            &kh("bob"),
            &NetworkPoint::from_socket(n.addr).unwrap(),
            &pop.txid,
    ...
    assert!(
        until(3000, || n
            ...
            .unwrap()
            .store
            .holds_txid(&peering.txid))
        .await,
        "N stored R's peering"
    );
    assert!(
        until(3000, || p
            .view
    ...
    assert!(
        c_view
            ...
            .peers_of(&kh("bob"))
            .contains(&kh("w2")),
        "C now holds R's edge"
    );
    assert_eq!(
        c_view.lock().unwrap().standing(&kh("w2")),
        10.0,
        "and R has standing at C, one edge beyond N"
    );
    ...
    let entry = anchor_entry(
        &id("w2"),
        &[NetworkPoint::from_socket(r.addr).unwrap()],
        0,
        Seqno {
            series: 1,
            counter: 1,
    ...
    assert!(
        n.anchors
            .lock()
            .unwrap()
            .offer(AnchorEntry::parse(&entry).unwrap(), &ids())
    );
    let req = ResolveRequest {
        subject: kh("w2"),
        anchor: kh("w2"),
        path: Path::empty().bytes,
        nibbles: 0,
        nonce: [5; 16],
    };
    let bytes = c
    ...
    let AttachOutcome::Attached(cr) = attach(&ccfg2, &client_ep(), kh("w2"), r.addr, false).await
    else {
        panic!("C reaches R")
    };
    let bytes = cr
    ...
    assert_eq!(
        ...
        "R holds no record of C and says so"
    );
    let bytes = cr
        .request(REQUEST_RESOLVE, &req.encode())
        .await
    ...
    assert!(
        matches!(
            ResolveReply::decode(&bytes).unwrap(),
            ResolveReply::Serving { .. }
        ),
        "R resolves itself directly"
    );
    drop(upstream);
    drop(r_to_n);
}
