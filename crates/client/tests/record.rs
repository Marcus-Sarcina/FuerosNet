...
fn a_presentation_withholds_every_field_by_default_and_the_digests_travel() {
    ...
    assert_eq!(
        p.withheld,
        LABELS.to_vec(),
        "every label's digest travels, in order"
    );
    assert_eq!(
        minimal.len(),
        rec.bytes.len() + 1 + 1 + 7 * 34,
        "envelope plus seven 32-byte digests and the array heads"
    );
    ...
    assert_eq!(
        p.revealed.keys().copied().collect::<Vec<_>>(),
        vec!["proximity"]
    );
    ...
    swapped.swap(0, 6);
    assert!(
        read_presentation(&ids(), &present(&rec.bytes, &swapped, &["capture"]))
            .unwrap_err()
            .contains("label")
    );
    ...
    assert!(
        read_presentation(
            &ids(),
            &present(&rec2.bytes, &lying_rec_set, &["proximity"])
        )
        .unwrap_err()
        .contains("strongest")
    );
    assert!(
        read_presentation(&ids(), &present(&rec2.bytes, &lying_rec_set, &[])).is_ok(),
        "withheld, the rule is unverifiable, not violated"
    );
    // an envelope signed by keys the reader lacks
    let without: Vec<_> = ids()
        .into_iter()
        .filter(|i| i.keyhash != kh("w2"))
        .collect();
...
fn a_subject_withholds_its_signature_from_a_body_omitting_a_response_it_holds() {
    ...
    assert_eq!(
        participant_check(&kh("alice"), &without, &nominees, &held, &hook),
    ...
    assert_eq!(
        participant_check(&kh("alice"), &with, &nominees, &held, &hook),
        Ok(())
    );
    // bob holds no response and signs either
    assert_eq!(
        participant_check(&kh("bob"), &with, &BTreeSet::new(), &BTreeMap::new(), &hook),
        Ok(())
    );
}
...
fn a_participant_refuses_a_witness_attributed_to_it_that_it_did_not_nominate() {
    ...
    assert_eq!(
        ...
        Err(Refusal::NomineeNotMine(kh("w3")))
    );
...
fn the_person_is_told_before_signing_when_their_nominees_are_outnumbered_or_absent() {
    let hook = Hook(RefCell::new(vec![]));
    let nominees = BTreeSet::from([kh("w1"), kh("w2")]);
    let mut witnesses: Vec<Witness> = ["w3", "w4", "c1", "c2"]
        .iter()
        .map(|n| Witness {
            keyhash: kh(n),
            nominated_by: kh("bob"),
    ...
    assert_eq!(
        ...
        &[Notice::NomineesOutnumbered { mine: 1, theirs: 4 }]
    );
    // absent altogether
    let none = Proposal {
    ...
    assert_eq!(
        participant_check(&kh("alice"), &none, &nominees, &BTreeMap::new(), &hook),
        Ok(())
    );
    assert_eq!(
        hook.0.borrow().as_slice(),
        &[Notice::NomineesOutnumbered { mine: 0, theirs: 4 }]
    );
    ...
    hook.0.borrow_mut().clear();
    assert_eq!(
        participant_check(&kh("alice"), &even, &nominees, &BTreeMap::new(), &hook),
        Ok(())
    );
...
fn a_witness_declines_a_claimed_start_far_from_its_clock() {
    ...
    assert_eq!(
        witness_check(observed + tolerance + 1, observed, tolerance),
        Err(Refusal::ClockFar {
            claimed: observed + tolerance + 1,
            observed
        })
    );
    assert_eq!(
    ...
    assert_eq!(
        witness_check(observed + tolerance, observed, tolerance),
        Ok(())
    );
    assert_eq!(
        witness_check(observed - tolerance, observed, tolerance),
...
fn a_late_response_is_kept_beside_its_record_and_goes_with_it() {
    ...
    let consented_q = VerificationQuery {
        subject: kh("alice"),
        querier: kh("bob"),
        ceremony_id: ceremony,
        profile: vec![1],
    ...
    let other_q = VerificationQuery {
        verifier: kh("c2"),
        ..consented_q.clone()
    };
    // a query consented in a different encounter, correctly signed
    let elsewhere = [8u8; 32];
    let elsewhere_q = VerificationQuery {
        ceremony_id: elsewhere,
        verifier: kh("c3"),
        ..consented_q.clone()
    };
    ...
    let response = |q: &VerificationQuery, v: &str| {
        Response {
            ...
            basis: Some(Basis::PhotoMatch),
            template_version: Some(1),
            consent: consent(&id("alice"), &q.query_id()),
            selection_basis: 1,
        }
        .sign(&id(v))
    };
    let good = LateResponse {
        record: rec.txid,
        subject: kh("alice"),
        response: response(&consented_q, "c1"),
    }
    .encode();
    let unconsented = LateResponse {
        record: rec.txid,
        subject: kh("alice"),
        response: response(&other_q, "c2"),
    }
    .encode();
    assert_eq!(
        take_late_response(&mut store, &ids(), &good, &consented),
        Ok(rec.txid)
    );
    assert!(
        take_late_response(&mut store, &ids(), &unconsented, &consented)
            .unwrap_err()
            .contains("countersign")
    );
    assert_eq!(store.late[&rec.txid], vec![good.clone()], "held beside R");
    assert_eq!(store.records[&rec.txid], rec.bytes, "R's bytes unchanged");
    assert_eq!(
        Record::parse(&store.records[&rec.txid]).unwrap().txid,
        rec.txid
    );
    // naming a record not held, or a subject who is not its participant
    let unknown = LateResponse {
        record: [1; 32],
        ..LateResponse::decode(&good).unwrap()
    }
    .encode();
    assert!(take_late_response(&mut store, &ids(), &unknown, &consented).is_err());
    let not_party = LateResponse {
        subject: kh("carol"),
        ..LateResponse::decode(&good).unwrap()
    }
    .encode();
    ...
    let cross = LateResponse {
        record: rec.txid,
        subject: kh("alice"),
        response: response(&elsewhere_q, "c3"),
    }
    .encode();
    assert!(
        take_late_response(&mut store, &ids(), &cross, &consented)
            .unwrap_err()
            .contains("this record's ceremony")
    );
    assert_eq!(
        store.late[&rec.txid].len(),
        1,
        "nothing was attached for it"
    );
    ...
    assert!(
        take_late_response(&mut bare, &ids(), &good, &consented)
            .unwrap_err()
            .contains("not resolvable")
    );
    assert!(
        !bare.late.contains_key(&rec.txid),
        "the evidence is not kept"
    );
    assert_eq!(
        bare.unattached_late[&rec.txid],
        vec![kh("c1")],
        "the arrival is, naming its verifier"
    );
    ...
    assert_eq!(
        bare.unattached_late[&rec.txid].len(),
        1,
        "still only the one that verified"
    );
...
fn a_presentation_a_decoder_must_refuse() {
    ...
    assert!(
        read_presentation(&ids(), &six)
            .unwrap_err()
            .contains("seven slots"),
        "six slots"
    );
    ...
    let at = shifted
        .windows(2)
        .position(|p| p == [0x50, 9])
        .expect("the sixteen-byte salt head");
    let mut fifteen = shifted.clone();
    fifteen[at] = 0x4f;
    fifteen.remove(at + 16);
    assert!(
        read_presentation(&ids(), &fifteen)
            .unwrap_err()
            .contains("salt"),
        "a fifteen-byte salt"
    );
    ...
    assert!(
        read_presentation(&ids(), &present(&rec2.bytes, &wrong, &["p0.retention"]))
            .unwrap_err()
            .contains("retention"),
        "a location where a retention belongs"
    );
    ...
    neither[n - 34] = 0x18;
    assert!(
        read_presentation(&ids(), &neither).is_err(),
        "a slot of some third shape"
    );

    // (e) and the envelope is still an envelope: a presentation that is
    // not an array of two is refused before any of this
    assert!(
        read_presentation(&ids(), &rec.bytes)
            .unwrap_err()
            .contains("presentation"),
        "a bare envelope is not a presentation"
    );
}
...
fn a_withheld_field_is_not_a_default_and_there_is_no_aggregate_verdict() {
    ...
    let p =
        read_presentation(&ids(), &present(&rec.bytes, &set, &["proximity"])).expect("well-formed");
    assert_eq!(p.revealed.len(), 1, "one field revealed");
    assert_eq!(
        p.revealed.get("p0.retention"),
        None,
        "a withheld field has no value here"
    );
    assert!(
        p.withheld.contains(&"p0.retention"),
        "it is named as withheld, which is a different fact"
    );
    assert!(
        p.withheld.contains(&"location"),
        "and withholding location is the default, not an absence of location"
    );
    ...
    let shown =
        read_presentation(&ids(), &present(&rec.bytes, &set, &["location"])).expect("well-formed");
    assert_eq!(
        shown.revealed["location"],
        empty_location_value(),
        "revealed and empty"
    );
    assert!(
        !shown.withheld.contains(&"location"),
        "which is not the same as withheld"
    );

    // **there is no aggregate verdict** (§4.5): the responses a record
    // carries are read one at a time, and nothing here collapses them.
    assert!(
        p.record.field_uint(5).is_none() || true,
        "field 5 is a list of responses, not a verdict"
    );
}
...
fn a_presence_record_takes_classical_response_signatures_and_a_recovery_takes_hybrid() {
    ...
    assert!(
        Record::parse(&rec.bytes).is_ok(),
        "a classical response is the presence shape"
    );
    ...
    assert!(
        e.contains("COSE_Sign1 in a presence record"),
        "refused for the signature's shape: {e}"
    );
    ...
    let under_signed =
        classical_signed_recovery_response(&id("w3"), &id("alice"), &qid, &kh("carol"));
    let block = tx::recovery_block(&id("carol"), &kh("alice"), &kh("bob"), vec![under_signed]);
    let a = tx::Adoption {
        node: kh("alice"),
        patron: kh("bob"),
        locator: tx::Locator::root(
            kh("bob"),
            tx::Seqno {
                series: 3,
                counter: 0,
        ...
        presented_head: None,
        back: [
            &[rhtn_archive::genesis(&kh("alice"))],
            &[rhtn_archive::genesis(&kh("bob"))],
        ],
    };
    let env = tx::envelope(
    ...
    let e = Record::parse(&env)
        .expect_err("a classical verifier signature does not survive a recovery");
    assert!(
        e.contains("hybrid COSE_Sign inside a recovery"),
        "refused for the signature's shape: {e}"
    );
}
...
fn a_witnesss_nominator_is_a_participant_and_the_signers_are_the_body_s_witnesses() {
    ...
    let e = Record::parse(&w.loose(
        TYPE_PRESENCE,
        &stranger.body(&back),
        &["alice", "bob", "w1", "w2"],
    ))
    .expect_err("carol is not a participant");
    ...
    .expect_err("carol witnesses nothing here");
...
fn classical_signed_recovery_response(
    verifier: &rhtn_crypto::SigningIdentity,
    subject: &rhtn_crypto::SigningIdentity,
    qid: &[u8; 32],
    prior: &rhtn_client::Keyhash,
) -> Vec<u8> {
...
fn a_client_saves_its_archive_and_its_store_and_comes_back_the_same_participant() {
    ...
    before
        .late
        .insert(rec.txid, vec![b"a late response".to_vec()]);
    ...
    assert_eq!(
        ...
        "the records come back byte for byte"
    );
    assert_eq!(
        after.seeds, before.seeds,
        "and the seeds, which are what release a capture key"
    );
    assert_eq!(
        after.sealed, before.sealed,
        "and the ciphertext this client cannot open"
    );
    assert_eq!(after.late, before.late);
    assert_eq!(after.unattached_late, before.unattached_late);
    assert_eq!(
        after.disclosures, before.disclosures,
        "salts and values, with the labels taken from their fixed order"
    );
    ...
    #[cfg(unix)]
    {
        ...
        let mode = std::fs::metadata(dir.join("seeds").join(&name))
            .unwrap()
            .permissions()
            .mode()
            & 0o777;
        assert_eq!(mode, 0o600, "the seed is not world-readable");
        let sealed_mode = std::fs::metadata(dir.join("sealed").join(&name))
            .unwrap()
            .permissions()
            .mode()
            & 0o777;
        assert_ne!(
            sealed_mode, 0o600,
            "ciphertext this client cannot open needs no such care"
        );
    }
    ...
    assert_eq!(
        ClientStore::load(&dir).expect("reads").records,
        before.records
    );
    let _ = std::fs::remove_dir_all(&dir);
}
