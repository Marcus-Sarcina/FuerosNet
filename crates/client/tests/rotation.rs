...
fn a_rotation_seals_every_old_line_as_the_old_keys_last_act() {
    ...
    for (patron, seal) in &seals {
        ...
        assert_eq!(
            ...
            "at the maximum counter"
        );
        assert_eq!(
            (loc.anchor, loc.path, loc.nibbles),
            (
                rel.position.anchor,
                rel.position.path.clone(),
                rel.position.nibbles
            ),
...
fn on_suspicion_each_line_is_sealed_then_reissued_naming_the_sealed_counter() {
    ...
    let t = w.clock + 60;
    let repairs = suspect_compromise(
        &id("alice"),
        &archive,
        t,
        &|p| w.archives[p].next_back_pointers(),
        &[9, 10, 11],
    )
    .unwrap();
    ...
    for r in &repairs {
        ...
        let old_series = relationships(&archive)
            .into_iter()
            .find(|x| x.patron == r.patron)
            .unwrap()
            .series();
        ...
        let sq_of = |k: u64| {
            let Some(Item::Array(a)) = map_get(m, k) else {
                panic!("field {k}")
            };
            Seqno {
                series: as_uint(&a[0]).unwrap() as u32,
                counter: as_uint(&a[1]).unwrap() as u32,
            }
        };
        assert_eq!(
            sq_of(3),
        ...
        assert_eq!(
            archive.chain_for(&r.patron).unwrap().series(),
            vec![old_series, r.new_series]
        );
    }
    ...
    assert_eq!(
        take_reissue(&mut archive, &r.reissue, &wrong, &ids()),
        Err(Refusal::NotTheProposal)
    );
}
...
fn a_reissue_never_enters_a_series_the_key_has_occupied() {
    ...
    take_reissue(
        &mut archive,
        &body,
        &envelope(TYPE_REISSUE, &body, &[&id("alice"), &id("bob")]),
        &ids(),
    )
    .unwrap();
    assert_eq!(
        archive.series_occupied().into_iter().collect::<Vec<_>>(),
        vec![1, 2, 3]
    );
    // asked to reissue back into s1 or s2: no such reissue is produced
    for used in [1, 2, 3] {
        assert_eq!(
            propose_reissue(&archive, &kh("bob"), &bob_back, 5, used, t),
            Err(Refusal::SeriesOccupied(used))
        );
    }
    ...
    assert_eq!(
        propose_reissue(&archive, &kh("w1"), &[], 5, 4, t),
        Err(Refusal::NoRelationship)
    );
}
