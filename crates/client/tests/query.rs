...
fn the_worked_query_and_its_frame_decode_and_the_consent_verifies() {
    ...
    assert_eq!(
        q.encode(),
        fixture("P-verification-query"),
        "re-encodes to the vector"
    );
    ...
    assert!(
        consent_verifies(&id("alice").public, &req.consent, &req.query.query_id()),
        "alice's consent over the id"
    );
...
fn the_grant_and_the_late_response_decode_and_round_trip() {
    ...
    assert_eq!(
        l.record,
        rhtn_codec::cose::txid(&body_of(&fixture("P-normal-record"))),
        "supplements the normal record"
    );
    let r = Response::read(&l.response).unwrap();
    assert_eq!(r.subject, l.subject);
    assert_eq!(r.verdict, Verdict::Inconclusive);
    verify::response(&ids(), &l.response, false)
        .expect("the late response verifies under its verifier");
...
fn a_response_signed_here_reads_back_and_verifies_like_the_records() {
    ...
    for rr in array_item_ranges(&body, r5.start).unwrap() {
        ...
        assert!(matches!(
            r.verdict,
            Verdict::Match | Verdict::NoMatch | Verdict::Inconclusive | Verdict::Unavailable
        ));
    ...
    assert!(
        verify::response(&vec![id("alice").public, id("carol").public], &bytes, false).is_err(),
        "not under another key"
    );
