...
fn responses_of(body: &[u8]) -> Vec<Vec<u8>> {
    ...
    array_item_ranges(body, r2.start)
        .unwrap()
        .into_iter()
        .map(|r| body[r].to_vec())
        .collect()
...
fn the_recovery_adoption_carries_both_halves_and_no_other_evidence_field() {
    ...
    assert!(
        value_slice(body, 6).is_some()
            && value_slice(body, 8).is_none()
            && value_slice(body, 9).is_none(),
        "field 6 and neither 8 nor 9"
    );
    ...
    let verifiers: Vec<_> = resps
        .iter()
        .map(|r| Response::read(r).unwrap().verifier)
        .collect();
    ...
    assert!(
        resps
            .iter()
            .any(|r| Response::read(r).unwrap().verdict == Verdict::Match)
    );
    ...
    assert!(
        verify::adoption_evidence(&ids(), &k1[rec.body.clone()]).is_err(),
        "names another successor"
    );
    ...
    verify::envelope(&ids(), &fixture("P-recovery-adoption"))
        .expect("the corpus's recovery adoption verifies here");
    let _ = at;
}
...
fn a_recovery_response_is_signed_hybrid_by_the_verifier_and_its_consent_classically_by_the_new_key()
{
    ...
    for bytes in responses_of(&env[rec.body.clone()]) {
        ...
        assert!(
            verify::response(&ids(), &bytes, false).is_err(),
            "not a classical Sign1"
        );
        ...
        assert_eq!(c.len(), 4);
        assert!(
            matches!(&c[3], Item::Bytes(_)),
            "a single signature, not entries"
        );
        assert!(rhtn_client::query::consent_verifies(
            &id("alice2").public,
            &bytes[r7.clone()],
            &r.query_id
        ));
        assert!(
            !rhtn_client::query::consent_verifies(&id("alice").public, &bytes[r7], &r.query_id),
            "not the old key"
        );
        assert_eq!(
            (r.verdict, r.basis, r.template_version),
            (Verdict::Match, Some(Basis::PersonalKnowledge), None)
        );
    }
}
...
fn the_recovery_meeting_is_a_ceremony_in_which_the_verifier_is_its_own_querier() {
    ...
    let q = log
        .iter()
        .find_map(|m| {
            if let Msg::ConsentRequest(q) = &m.msg {
                Some(q.clone())
            } else {
                None
            }
        ...
        .expect("a query");
    assert_eq!(
        (q.querier, q.verifier, q.subject),
        (kh("bob"), kh("bob"), kh("alice2"))
    );
    let intents: Vec<_> = log
        .iter()
        .filter_map(|m| {
            if let Msg::Intent(i) = &m.msg {
                Some((m.from, i.contribution))
            } else {
                None
            }
        })
        .collect();
    let (ca, cb) = (
        intents.iter().find(|(f, _)| *f == kh("alice2")).unwrap().1,
        intents.iter().find(|(f, _)| *f == kh("bob")).unwrap().1,
    );
    assert_eq!(
        q.ceremony_id,
        rhtn_client::keys::pre_commitment((&kh("alice2"), &ca), (&kh("bob"), &cb)),
        "the meeting's pre-commitment"
    );
    // S's new key countersigned it
    let consent = log
        .iter()
        .find_map(|m| {
            if let Msg::Consent { consent, .. } = &m.msg {
                Some(consent.clone())
            } else {
                None
            }
    ...
    assert_eq!(
        (r.selection_basis, r.subject, r.verifier, r.query_id),
        (0, kh("alice2"), kh("bob"), q.query_id())
    );
    ...
    assert!(
        s.prompts_asked("bob")
            .iter()
            .any(|q| q.starts_with("Do you recognise"))
    );
    // and a verifier who never met the prior key asks nobody and answers nothing
    s.face_off("alice2", "carol");
    assert_eq!(
        s.h.run_recovery_meeting(kh("alice2"), kh("carol")),
        Err(Abort::NotRecognised)
    );
    assert!(
        s.prompts_asked("carol")
            .iter()
            .all(|q| !q.starts_with("Do you recognise"))
    );
}
...
fn the_old_lines_are_sealed_before_the_recovery_adoption_goes_anywhere() {
    ...
    let seal_at = log
        .iter()
        .position(|m| matches!(m.msg, Msg::Seal(_)))
        .expect("a seal");
    let record_at = log
        .iter()
        .position(|m| matches!(m.msg, Msg::Record(_)))
        .expect("the record");
    let proposal_at = log
        .iter()
        .position(|m| matches!(m.msg, Msg::RecoveryProposal { .. }))
        .unwrap();
    assert!(
        proposal_at < seal_at && seal_at < record_at,
        "statement, then seals, then the adoption"
    );
    let Msg::Seal(seal) = &log[seal_at].msg else {
        unreachable!()
    };
    let sl = SignedLocator::parse(seal).unwrap();
    assert_eq!(
        (sl.subject, sl.locator.seqno.counter, log[seal_at].to),
        (kh("alice"), COUNTER_MAX, kh("w1"))
    );
    assert_eq!(
        s.h.clients[&kh("alice2")].prior_key(),
        None,
        "nothing signed by the old key follows"
    );
}
