...
fn the_corpus_catalog_entry_and_abuse_report_read_and_verify_and_their_forgeries_do_not() {
    ...
    assert_eq!(
        (e.owner, e.service_type.as_str(), e.instance.as_str()),
        (w.kh("bob"), "rhtn-forum", "The Reading Room")
    );
    ...
    assert_eq!(
        (a.resource, a.owner, a.endpoint, a.data_practice),
        (b.resource, b.owner, b.endpoint, b.data_practice)
    );
    // the abuse report: by the resource it names, and not by another key
    let r = AbuseReport::parse(&fixture("P-abuse")).unwrap();
    assert_eq!(
        (r.category, r.detail.as_deref()),
        (ABUSE_EXCESSIVE_LOAD, Some(&b"burst of 9k requests/min"[..]))
    );
...
fn the_scope_forms_and_the_frames_and_replies_round_trip() {
    ...
    assert_eq!(
        Scope::decode(&fixture("P-scope-down2")).unwrap(),
        Scope::Down(2)
    );
    assert_eq!(Scope::Down(2).encode(), fixture("P-scope-down2"));
    assert!(
        matches!(Scope::decode(&fixture("B-scope-256")).unwrap(), Scope::List(l) if l.len() == 256)
    );
    ...
    assert_eq!(
        (reg.scope.clone(), reg.entry.clone()),
        (None, fixture("P-catalog"))
    );
    ...
    assert_eq!(
        (tr.entries.len(), tr.continuation.as_deref()),
        (111, Some("rhtn-wiki"))
    );
    let r8 = ResourceResponse::decode(&fixture("P-reply-08")).unwrap();
    assert_eq!(
        (r8.status, r8.body.as_deref()),
        (
            STATUS_DELIVERED,
