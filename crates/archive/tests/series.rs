...
fn a_reissue_naming_a_series_already_in_the_chain_is_rejected_and_an_unused_one_extends_it() {
    ...
    let r1 = w.reissue("bob", "alice", Seqno { series: 1, counter: 5 }, 2);
    let ids = w.lookup();
    ...
    assert_eq!((chain.current(), chain.depth(), chain.series()), (2, 2, vec![1, 2]));
    // a patron-countersigned reissue back into s1 is rejected, and the chain is unchanged
    let back = w.reissue("bob", "alice", Seqno { series: 2, counter: 3 }, 1);
    ...
    let s3 = w.reissue("bob", "alice", Seqno { series: 2, counter: 4 }, 3);
    chain.take_reissue(&s3, &ids).unwrap();
    assert_eq!((chain.current(), chain.depth()), (3, 3));
    assert_eq!(chain.abandoned().into_iter().collect::<Vec<_>>(), vec![1, 2]);
    // a reissue leaving a series other than the current one is not this chain's next link
    let stray = w.reissue("bob", "alice", Seqno { series: 1, counter: 9 }, 4);
    assert!(chain.take_reissue(&stray, &ids).unwrap_err().contains("other than the current"));
    // nor is another relationship's
    let f2 = w.meet("bob", "carol");
    let _ = w.adopt("bob", "carol", f2.txid, 7);
    let other = w.reissue("bob", "carol", Seqno { series: 7, counter: 1 }, 8);
    assert!(chain.take_reissue(&other, &ids).unwrap_err().contains("another relationship"));
...
fn two_chains_from_one_adoption_that_diverge_are_equivocation() {
    ...
    let a = w.adopt("bob", "alice", f.txid, 1);
    let ids = w.lookup();
    let x = w.reissue("bob", "alice", Seqno { series: 1, counter: 5 }, 2);
    let y = w.reissue("bob", "alice", Seqno { series: 1, counter: 6 }, 3);
