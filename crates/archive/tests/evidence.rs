...
fn body_err(body: &[u8]) -> String {
    check_body(body, &rhtn_codec::cbor::parse_all(body).unwrap())
        .unwrap_err()
        .0
        .to_string()
}
...
fn an_adoption_carries_one_evidence_form_and_a_recovery_names_its_parties() {
    ...
    assert!(
        parsed(&w, TYPE_ADOPTION, &none, &["alice", "bob"])
            .unwrap_err()
            .contains("exactly one of fields 6, 8 and 9"),
        "no evidence"
    );
    ...
    assert!(
        parsed(&w, TYPE_ADOPTION, &both, &["alice", "bob"])
            .unwrap_err()
            .contains("exactly one of fields 6, 8 and 9"),
        "two forms"
    );
    ...
    let recovery = |old: &rhtn_crypto::SigningIdentity, responses: Vec<Vec<u8>>| {
        Evidence::Recovery(recovery_block(
            old,
            &new.public.keyhash,
            &bob.public.keyhash,
            responses,
    ...
    let refused = |ev: Evidence| {
        parsed(
            &w,
            TYPE_ADOPTION,
            &adoption_bytes(&w, "alice2", "bob", ev, 1),
            &["alice2", "bob"],
        )
        .unwrap_err()
    };
    // a response attesting continuity to a third party
    let e = refused(recovery(
        old,
    ...
    let e = refused(recovery(
        old,
        vec![recovery_response(new, new, &qid("q4"), &old.public.keyhash)],
    ));
    ...
    let e = refused(recovery(
        ...
        vec![
            good.clone(),
            recovery_response(carol, new, &qid("q5"), &old.public.keyhash),
        ],
    ));
...
fn a_formation_is_genesis_rooted_and_a_normal_record_is_witnessed() {
    ...
    let f = formation_body(
        [&[genesis(&ka)], &[genesis(&kb)]],
        [&ka, &kb],
        t,
        t + 600,
        &root,
    ...
    assert!(
        parsed(&w, TYPE_PRESENCE, &f_bad, &["alice", "bob"])
            .unwrap_err()
            .contains("genesis"),
        "non-genesis formation"
    );
    ...
    assert_eq!(
        second.field_uint(6),
        Some(0),
        "an established key meets on a normal record"
    );
    assert_eq!(second.signers.len(), 3, "witnessed");
    let f2 = formation_body(
        [&w.back("alice"), &w.back("carol")],
        [&ka, &kc],
        t,
        t + 600,
        &root,
    ...
    assert!(
        parsed(&w, TYPE_PRESENCE, &f2, &["alice", "carol"])
            .unwrap_err()
            .contains("genesis"),
        "a second formation for alice"
    );
    ...
    assert!(
        body_err(&with_uint(normal, 6, 1)).contains("no witnesses"),
        "formation with witnesses"
    );
    // (d) a formation's subtype flipped to normal: no witnesses, and a
    // normal record carries one to sixteen
    assert!(
        body_err(&with_uint(&f, 6, 0)).contains("carries witnesses"),
        "normal record without witnesses"
    );
    // (e) the two participants one identity
    let same = formation_body(
        [&[genesis(&ka)], &[genesis(&ka)]],
        [&ka, &ka],
        t,
        t + 600,
        &root,
    ...
    assert!(
        body_err(&same).contains("participant identities are one"),
        "equal participants"
    );
}
...
fn a_body_is_checked_against_the_type_its_envelope_names() {
    ...
    assert!(
        parsed(&w, TYPE_ADOPTION, &dep, &["bob", "alice"])
            .unwrap_err()
            .contains("locator"),
        "a departure body in an adoption envelope"
    );
    // (b) every two-party type refuses one identity in both roles
    let one = body_err(&departure_body(
        &w.back("bob"),
        &kb,
        &kb,
        Seqno {
    ...
    let one = body_err(&reissue_body(
        [&w.back("bob"), &w.back("bob")],
        &kb,
        &kb,
        Seqno {
            series: 1,
    ...
    let one = body_err(&adoption_bytes(
        &w,
        "bob",
        "bob",
    ...
    assert!(
        body_err(&peering(false)).contains("field 8"),
        "peering without evidence"
    );
}
...
fn nested_signed_structures_are_checked_inside_a_body() {
    ...
    let stray = presence_record_body(
        &backs,
        [&ka, &kb],
        &[Witness {
            keyhash: kc,
    ...
    let nominated = presence_record_body(
        &backs,
        [&ka, &kb],
        &[Witness {
            keyhash: kc,
    ...
    assert!(
        parsed(&w, TYPE_PRESENCE, &nominated, &["alice", "bob", "carol"]).is_ok(),
        "nominated by a participant"
    );
    ...
    assert!(
        check_body(
            &peering(&[127, 0, 0, 1]),
            &rhtn_codec::cbor::parse_all(&peering(&[127, 0, 0, 1])).unwrap()
        )
        .is_ok()
    );
...
fn a_missing_embedded_signers_key_is_the_third_outcome_and_names_the_key() {
    ...
    let without = |name: &str| -> Vec<rhtn_crypto::Identity> {
        w.lookup()
            .into_iter()
            .filter(|i| i.keyhash != w.kh(name))
            .collect()
    };
    // the former patron's key absent: neither verified nor rejected, and
    // the identity to fetch is named
    assert_eq!(
        xfer.check_signatures(&without("carol")),
        SigStatus::Unverifiable {
            missing: w.kh("carol")
        }
    );
    ...
    let rec = Evidence::Recovery(recovery_block(
        old,
        &new.public.keyhash,
        &bob.public.keyhash,
        vec![resp],
    ...
    assert_eq!(
        rec.check_signatures(&without("carol")),
        SigStatus::Unverifiable {
            missing: w.kh("carol")
        }
    );
    assert_eq!(
        rec.check_signatures(&without("alice")),
        SigStatus::Unverifiable {
            missing: w.kh("alice")
        }
    );
...
fn a_transfer_whose_former_patron_is_a_participant_is_malformed() {
    ...
    assert!(
        parsed(&w, TYPE_ADOPTION, &body, &["alice", "bob"]).is_err(),
        "and it never reaches a signature check"
    );
...
fn a_disavowals_reason_is_an_enumerated_code_inside_a_space_of_sixty_four() {
    ...
    for c in [None, Some(0), Some(31), Some(32), Some(63)] {
        assert!(
            parsed(&w, TYPE_DISAVOWAL, &body(c), &["alice"]).is_ok(),
            "code {c:?} is inside the space"
        );
    }
    assert!(
        body_err(&body(Some(64))).contains("out of space"),
        "64 is one past the end"
    );
    ...
    assert!(
        body_err(&prose).contains("code uint"),
        "a reason that is not a uint"
    );
}
...
fn a_departure_countersigned_by_the_patron_is_refused_and_so_is_a_disavowal() {
    ...
    assert!(
        parsed(&w, TYPE_DEPARTURE, &dep, &["bob"]).is_ok(),
        "one signature is the shape"
    );

    // the patron countersigning it: two logical signers is four entries,
    // over the ceiling §1.3 derives for a one-signer type
    let both = parsed(&w, TYPE_DEPARTURE, &dep, &["bob", "alice"])
        .expect_err("two signers on a unilateral type");
    assert!(
        both.contains("ceiling"),
        "refused for the entry count: {both}"
    );

    // and the patron signing it instead of the node, which is the same
    // count and a different party: the body names who must sign
    let wrong = parsed(&w, TYPE_DEPARTURE, &dep, &["alice"])
        .expect_err("the patron is not the departing node");
    ...
    assert!(
        parsed(&w, TYPE_DISAVOWAL, &dis, &["alice"]).is_ok(),
        "the patron alone"
    );
    let both = parsed(&w, TYPE_DISAVOWAL, &dis, &["alice", "bob"])
        .expect_err("two signers on a unilateral type");
    assert!(
        both.contains("ceiling"),
        "refused for the entry count: {both}"
