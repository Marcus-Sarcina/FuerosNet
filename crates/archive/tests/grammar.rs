...
fn a_structured_field_is_the_object_the_grammar_names() {
    ...
    let good = PrekeyPublication {
        bundle: b.clone(),
        nonce: [1; 16],
    }
    .encode();
    let item = parse_all(&good).expect("parses");
    let Item::Map(ref m) = item else {
        panic!("a map")
    };
    assert!(
        matches!(map_get(m, 1), Some(Item::Map(_))),
    ...
    assert!(
        PrekeyPublication::decode(&wrapped).is_err(),
        "the wrapping is refused"
    );
    ...
    assert!(
        rhtn_codec::schema::check_unsigned(rhtn_codec::schema::Family::PrekeyPublication, &good, 0)
            .is_ok()
    );
    assert!(
        rhtn_codec::schema::check_unsigned(
            rhtn_codec::schema::Family::PrekeyPublication,
            &wrapped,
            0
        )
        .is_err()
...
fn a_value_outside_its_declared_range_is_refused_not_narrowed() {
    ...
    assert_eq!(
        (back.seqno.series, back.seqno.counter),
        (u32::MAX, u32::MAX)
    );
    ...
    for (series, counter) in [(1u64 << 32, 0u64), (0, 1u64 << 32)] {
        ...
        assert_ne!(
            sent as u32 as u64, sent,
            "narrowing changes the value, which is why it is refused"
        );
    }
}
...
fn a_conditional_field_stands_or_falls_with_the_field_it_describes() {
    ...
    assert_eq!(
        WakeRegistration::decode(&registered)
            .unwrap()
            .endpoint
            .as_deref(),
        Some("https://push.example/rhtn/a3f9")
    );
    ...
    let orphan_key = WakeRegistration {
        nonce,
        endpoint: None,
        key: Some(vec![7; 32]),
        lapses_at: None,
    ...
    .encode();
    assert!(
        WakeRegistration::decode(&orphan_key).is_err(),
        "a key describes an endpoint that is not there"
    );
    // a lapse with no endpoint either
    let orphan_lapse = WakeRegistration {
        nonce,
        endpoint: None,
        key: None,
        lapses_at: Some(1_800_090_000),
    }
    .encode();
    assert!(
        WakeRegistration::decode(&orphan_lapse).is_err(),
        "and so does a lapse"
    );
    // an endpoint with nothing to encrypt the body to
    let no_key = WakeRegistration {
        nonce,
        endpoint: Some("https://push.example/x".into()),
        key: None,
        lapses_at: None,
    }
    .encode();
    assert!(
        WakeRegistration::decode(&no_key).is_err(),
        "an endpoint nothing can be sent to"
    );
    ...
    for bytes in [&orphan_key, &orphan_lapse, &no_key] {
        assert!(
            rhtn_codec::schema::check_unsigned(
                rhtn_codec::schema::Family::WakeRegistration,
                bytes,
                0
            )
            .is_err()
