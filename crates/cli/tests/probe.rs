...
fn ids() -> Vec<rhtn_crypto::Identity> {
    ["alice", "bob", "carol", "witness"].iter().map(|n| test_identity(n).public).collect()
}
...
fn run(args: &[&str]) -> (bool, String, String) {
    let out = Command::new(env!("CARGO_BIN_EXE_rhtn")).args(args).output().expect("rhtn runs");
...
async fn a_probe_resolves_fetches_an_archive_and_queries_a_catalog_over_a_real_session() {
    ...
    let pop = {
        ...
        let w = Witness { keyhash: kh("witness"), nominated_by: kh("bob"), flags: 3 };
        let body = presence_record_body(&[g("bob"), g("carol"), g("witness")], [&kh("bob"), &kh("carol")], &[w], 1_800_000_000, 1_800_000_600, &root);
        envelope(TYPE_PRESENCE, &body, &[&test_identity("bob"), &test_identity("carol"), &test_identity("witness")])
    };
    ...
    let adoption = {
        let a = Adoption {
            node: kh("carol"),
            patron: kh("bob"),
            locator: Locator { anchor: kh("bob"), path: path.bytes.clone(), nibbles: path.nibbles, seqno: Seqno { series: 1, counter: 0 } },
        ...
        envelope(TYPE_ADOPTION, &adoption_body(&a), &[&test_identity("carol"), &test_identity("bob")])
    };
    let mut table = Table::with_me(kh("bob"));
    table.mark_infra(kh("bob"));
    let store = std::collections::BTreeMap::from([(pop_id, pop.clone())]);
    table.apply(&Record::parse(&adoption).unwrap(), &ids(), &store, None).expect("applies");
    let mut view = NodeView::new(Arc::new(test_identity("bob")), Locator { anchor: kh("bob"), path: Vec::new(), nibbles: 0, seqno: Seqno { series: 1, counter: 0 } });
    view.table = table;
    ...
    let node = LiveNode::start(cfg, view, ids(), AnchorTable::new(0, Ingestion::UnverifiedGossip));
    ...
    assert!(out.contains("ServingInfra") || out.contains("Referral") || out.contains("Failure"), "an answer of some shape: {out}");
    // archive fetch: the two records bob holds, head first
    let (ok, out, err) = run(&[&base[..], &["archive", &target]].concat());
    assert!(ok, "archive: {err}");
    assert!(out.contains("records   2"), "the two records bob signed: {out}");
    assert!(out.contains(&hex(&Record::parse(&adoption).unwrap().txid)), "naming the adoption: {out}");
    // catalog: an empty catalog answers, and answering is the point
    let (ok, out, err) = run(&[&base[..], &["catalog"]].concat());
    assert!(ok, "catalog: {err}");
    assert!(out.contains("entries   0"), "an empty catalog is an answer: {out}");
    let _ = std::fs::remove_dir_all(&dir);
}
...
async fn an_archive_batch_that_is_not_a_chain_is_refused_rather_than_reported() {
    ...
    cfg.on_request = Some(Arc::new(move |_peer, family, body| {
        ...
        Box::pin(async move {
            ...
            Some(ArchiveReply { nonce: req.nonce, records, more: false, frontier: Vec::new() }.encode())
        })
    }));
