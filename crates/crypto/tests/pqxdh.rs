...
fn keys(tag: u8) -> (DhSecret, DhSecret, KemSecret, DhSecret, KemSecret) {
    (
        DhSecret::from_seed([tag; 32]),
        DhSecret::from_seed([tag + 1; 32]),
        KemSecret::from_seed([tag + 2; 64]),
        DhSecret::from_seed([tag + 3; 32]),
        KemSecret::from_seed([tag + 4; 64]),
    )
}
...
fn both_parties_reach_one_secret_with_and_without_one_time_keys() {
    ...
    let (ik_b_pub, spk_b_pub, pqspk_b_pub, opk_b_pub, pqopk_b_pub) = (
        ik_b.public(),
        spk_b.public(),
        pqspk_b.public(),
        opk_b.public(),
    ...
    // with one-time keys
    let their = TheirBundle {
        ik: &ik_b_pub,
        spk: &spk_b_pub,
        pqspk: &pqspk_b_pub,
        opk: Some(&opk_b_pub),
        pqopk: Some(&pqopk_b_pub),
    };
    ...
    assert_eq!(
        respond(&me0, &ik_a.public(), &init0.ek, &init0.kem_ciphertext).unwrap(),
        init0.sk
    );
    assert_ne!(init0.sk, init.sk);
    // the responder using a one-time key the initiator did not: disagreement
    assert_ne!(
        respond(&me, &ik_a.public(), &init0.ek, &init0.kem_ciphertext).unwrap(),
        init0.sk
    );
    ...
    ct[0] ^= 1;
    assert_ne!(
        respond(&me, &ik_a.public(), &init.ek, &ct).unwrap(),
        init.sk
    );
    // another initiator identity: disagreement
    let ik_x = DhSecret::from_seed([99; 32]);
    assert_ne!(
        respond(&me, &ik_x.public(), &init.ek, &init.kem_ciphertext).unwrap(),
        init.sk
    );
