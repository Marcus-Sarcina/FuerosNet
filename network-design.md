...
- **The patron was not there.** A signature adds no evidence about a physical
  ...
  here. PoP is already rate-limited by human time (§7.1), which is a tighter
  constraint than a patron could impose.
...
7. **Each party reviews the other's selection before signing.** Because A selects
   ...
   repair it, since the record is immutable. This is one of the ceremony's checks that
   protect a signer **against their counterparty** rather than against outsiders
   or against the pair colluding; `light-client-requirements.md` §1.4 counts
   them.
...
- **Consent: the subject countersigns every query about themselves.** *No
  ...
  **The premise is the honest counterparty, and it excludes the malicious one.**
  ...
  about themselves will withhold it. **The subject sees it because the verifier
  delivers it** [author, 2026-09-03]: a verifier answering a query about subject
  S sends a copy of its signed response to S as well as to the querier — the
  verifier already holds a point-to-point association with S for the capture-key
  grant (§7.5.2), so S is identifiable and reachable, and returning the verdict
  adds one message to an exchange that exists anyway. S consented to the query,
  so the copy disclosed nothing. Without this leg the sentence above was an
  assumption: responses returned only to the querier, and a substituting querier
  could omit the `no_match` and present a merely thin body for signature.
  Registered rather than closed — binding a profile
...
**The derivation is HKDF-SHA-256, exactly** [2026-09-02]: salt empty, IKM the
seed, info the ASCII tag `rhtn/1:capture` followed by the raw 32-byte subject
and holder keyhashes and the raw 32-byte ceremony pre-commitment, output
length 32. Stated to the byte because any divergence derives a different key
and the holder cannot decrypt what it sealed.
...
This is consistent with what §7 already says — proof of presence is **a cost, not
...
signed (§8.1's `corroborated[]`), so a later evaluator sees which corroborations
...
§5.1 and `wire-format.md` §1. `H()` denotes the canonical hash.
...
Carrying A's signature in the record means C cannot fabricate it, and **the
dual delivery of §7.4.2 means C cannot suppress it either while B is honest**
[author, 2026-09-03]: A's response reaches B as well as C, and B withholds the
...
evaluator; §19.2 treats the resulting social-graph leakage as an open finding.
...
- **Where the bundles surface no common acquaintance, the parties go
  ...
  to extract more names is visible to the party being fished.
...
"this is the same person" fact, by photo match or personal knowledge (§8.1's
...
typical case a verifier outside the user's last active trust horizon cannot
...
| A nearby node in your trust horizon | Local | Modest |
...
both inside the same trust horizon, leaves shared neighbours holding two claims and
...
- **A fork can be invisible to the forked-from.** With a password you notice when
  ...
  The rotation *delta* is pushed within horizon, because the neighbours there need
  it to keep routing; the currency *state* is queried by everyone else on
...
to its owner's trust horizon, and that is not a ceiling on what can be built: a
...
**nothing reaches outside it**. Present encoding: membership in the owner's trust
horizon, which is the region a node holds topology for (§15.1). Consequences:
...
Adoption puts a node inside its grandpatron's trust horizon, so the §11.2 gate opens
...
evaluating the new node themselves.** Which nodes is settled by how far the
acknowledgement travels: it is forwarded as far as the acknowledged node's own
adoption (`wire-format.md` §7.5 and `wire-format.md` §10.1.1), so the acceptors
are the nodes within
that reach, in practice the patron's siblings, who hold the new node in their
horizon but have no relationship with it. A node further away, the
grandpatron's sibling or the great-grandpatron, never holds the acknowledgement
and evaluates the new node for itself [author, 2026-09-23].
...
| Who | Effect |
|---|---|
| **The old upline and lateral org.** Former patron and its generation, former siblings, and their subordinates | **Lose access.** They are no longer within the owner's trust horizon |
...
Scope =
  ...
  | dunbar                ; the owner's whole trust horizon (§15.1). A primitive,
                          ;   NOT a subtree — no subtree has this shape
  | list([keyhash])       ; named members, inside the trust horizon like every
                          ;   other scope (§11.2)
```
...
**No scope reaches outside the owner's trust horizon**, `list` included. It escapes
...
enumerate what a resource can do. That is the application's business (`resource-requirements.md` §1). Two
...
of them. Policy resides at the owner's node, evaluated against the role table
§11.4 materialises, which is the party that can genuinely enforce.
...
trust horizon and the roles that member has. **Authorisation at request time is a
lookup**, not an evaluation.

**The table is bounded, so materialising it costs nothing.** A trust horizon is 221
...
CatalogEntry = {
  ...
  connection      : opaque         ; how to reach it; the SRV analogue
  metadata        : ? opaque       ; additional service metadata, the TXT
                                   ;   analogue (`wire-format.md` §6.1)
  discover_scope  : Scope          ; NOT PART OF THE ENTRY, and not encoded in
                                   ;   it. The answering node's rule for which
                                   ;   askers it returns this entry to —
                                   ;   requested at registration, never sent to
                                   ;   an asker (§11.5)
  connect_scope   : ? Scope        ; who may open a session; advisory, and
                                   ;   absent offers no prediction
...
which is a property of who answers rather than of who forwards. This is capability-style cataloguing: §11.2's access
...
**The protocol defines no response.** What happens next is the owner's, decided
against the role table its own node materialises (§11.4).
...
### 12.1 Locator format
A locator is three fields — a topological position, with no address in it:

```
{ anchor, path, sequence }
```
...
**Authentication is non-optional, and which form supplies it depends on how the
locator travels.** **Any routing information presented as specifying where a
participant can be reached must be authenticated by that participant**, so an
intermediary cannot substitute itself as the destination; without that, any
...
- Anchor count ≈ N/S. **Stress test** (not a design target): at a deliberately
  ...
  which at ~50 bytes an entry is ~6 MB. The figure exists to prove the mechanism does
...
- **Entry format (key hash, not key):** 32B hash + a `NetworkPoint` of 6B or so
  — a four-byte IPv4 address and a port, since a 16-byte address is malformed in
...
- **Anchor table.** Cached per local policy and updated by gossip. **Not
  ...
  (§12.2, §12.7.3).
- **Trust horizon (two-edge walk, §15.1).** Pre-fetched and kept warm; this is where most
  traffic goes.
...
as before. **The disclosure this makes is its own finding** (§19.4, P40): the
exposure stops being bounded by the horizon and becomes chosen instead, which
is a different shape of cost rather than none.
...
trust horizon, where no anchor is involved at all.
...
already rest on it — a witness declines a ceremony dated far from its own
clock (`light-client-requirements.md` §1.2). What §3.3 withholds is *global* sequencing and reader-dependent
validity, not local time. **What is worth naming is that this particular use
is a security decision.** The error is asymmetric: a clock running fast
rejects live staples and costs availability, while a clock running slow
accepts dead ones, which is what an adversary able to influence it would
choose. A party has no second source to check its clock against, so neither
...
parser beside the first, which is the hazard `wire-format.md` §11.2 names for HTTP at a
...
Present encoding: reusable prekey material is prefetched across the trust horizon as a
...
**This is the horizon's eighth job** (§15.1): prefetch scope. Outside the org the
...
reach* is appropriate; a pattern says *how* that reach is achieved. Three patterns
are in use:
...
adoption transaction (§3.4). Over adoption edges alone the same walk yields 122 and
...
| **Resource access eligibility** | §11.2 — membership in the owner's trust horizon gates all resource access |
...
- **Adoption edges — the routing and authority hierarchy.** These carry
  ...
  transaction rather than by one of their own (§3.4), and they add no
...
available to that class by other means. And a finding that an attacker holding a
device sees the local archive is not by itself a defect — the question is whether
it costs more than reading an ordinary phone. Findings that clear the floor are
recorded because they are real, not because they are failures.
...
**End-to-end payload encryption is adopted rather than designed here** — PQXDH
and the Triple Ratchet, §14.2.4 — with integration decisions open, which is the
residual risk P12 records. The analysis below describes the exposure in an
...
infra node, so **without §14.2.4 both serving infra nodes would see payload
plaintext**. §14.1.6 once listed "end-to-end encryption of queued payload"
among things the queue policy had to settle; §14.2.4 settled it, §19.5's table
records where, and what remains open at §14.1.6 is the cap's value alone.

§12.6.3 already accepts the patron as a *metadata* chokepoint. It was never
intended to be a *content* chokepoint, and **§14.2.1 is where the two are
reconciled** — as a relay the patron sees ciphertext, as an endpoint it reads
plaintext, and a message is one or the other.
...
| **P12** | **End-to-end payload encryption is adopted rather than designed here, and an implementation shipping hop encryption alone leaks payload to both serving nodes** | **Critical for any implementation without it** | §14.2.4 adopts PQXDH and the Triple Ratchet; five integration decisions remain (§22.1). The patron was accepted as a metadata chokepoint, never a content one |
...
2. **Locator topology is disclosed on introduction, and is not meant to be secret.**
   ...
   **Only the natural person can bridge their positions and regulate between them**,
   ...
   multiple-identity support is the deferred piece that completes this (§13.7,
   and P3 in §19.4). And
...
6. **Visible infrastructure placement** — the ASN is exposed *so that*
   ...
   on (§1.2.3).
7. **Optional platform-vendor metadata. Superseded by item 13**, and the number
   is kept so a citation resolves here. The doorbell is posted to an endpoint the
   user's own service supplies and no vendor credential exists at the node
   (§14.1.5), so there is no platform vendor left to price; and §14.1.4 does not
   read the choice as a degradation of the trust model.
...
threats were largely already registered in §19.4, and that **the new material was
...
| **C23** | Every request class one serving node answers, kept as histories | **An interest graph** [author, 2026-09-18]. A resolution says whom a client wants to reach (P26), a one-time key request that a session is opening (C11), a filtered catalog query what kind of service it seeks (P38), a currency fallback whom it is evaluating (§19.7 item 12), a publication or deposit that it is making itself reachable. Each is priced alone, and one node answers all of them, so histories of them join into whom a client looks for, listens to and asks about, which no single class yields. §19.1's rule applied to the serving node: the mitigation is one obligation rather than one per class, the state serving needs and no history of the requests that produced it (`infra-client-requirements.md` §1) | High for a node keeping histories; the obligation is what keeps it low |
| **C24** | A candidate's IP handed over at a ceremony + the presence the two parties already share | **No location that was not disclosed by standing there, and possibly the network operator** [author, 2026-09-25]. The join that looks alarming is not one: §7.6 makes an IP a coarse location, but this counterparty was in the room, so the geography it yields is geography they already have. P40 therefore adds nothing to C1's third ingredient, and C1's withholding mitigation (§8.1.1) is not weakened by it. **What the address can add is the carrier or ISP behind it**, which physical presence does not imply, which the user carries between places, and which is a handle for parties other than the counterparty. Light clients are recommended a transport order that minimises it (`light-client-requirements.md` §4) | Low |
...
targets for simulation; the last column says what evidence would settle each. **Numbers are never reused**, so the gap at A20 is real
and a citation to it resolves to `change-log.md`, where it is recorded withdrawn;
A23 is superseded in place by A34.

| # | Assumption | What rests on it | If false | Evidence that would settle it |
|---|---|---|---|---|
| **A1** | **Human time is the binding resource for a presence-proof attack** | The entire Sybil defence (§17.3). Ceremony duration is priced in minutes precisely to meter it | If attackers can hire humans at scale cheaply, presence proofs are far weaker than §17.3 claims and the three-legged defence becomes two-legged | A measured attack-cost model comparing human ceremony time with capital/automation scaling, including wages, concurrency, collusion, and ceremony throughput. |
| **A2** | **Most traffic stays within the two-edge trust horizon** | The control/payload split (§12.6.3), f=10 affordability (§3.2), rarity of anchor lookups (§12.2), store-and-forward sufficiency (§14.1.4), **and now the direct payload path, which is horizon-limited (§12.6.3)** | Apex load, anchor load and push requirements change together, **and relay load becomes (out-of-horizon traffic) + (in-horizon traffic where traversal fails)**, so if this is wrong the infra economics of §16.6 collapse as well. The single most load-bearing behavioural claim in the document | Traffic telemetry or a representative workload model showing what fraction of payload/control activity falls inside a two-edge horizon. |
| **A3** | **Apex load scales with churn, not usage** | The f=10 cap surviving at scale (§3.2) | The fanout cap becomes a throughput ceiling and the social rationale collides with the plumbing again | A queueing/load model or simulation separating churn-driven control traffic from usage-driven traffic across the hierarchy. |
| **A4** | **Evaluation demand is much smaller than total activity** | Pull-not-push attestation (§15), the biggest scaling decision here | Backbone traffic grows with population. Note this is now framed as an implementation constraint (§15), which is enforceable — unlike the others | Measurements or a workload model comparing evaluation/query demand with total network activity. |
| **A5** | **Presence ceremonies are rare per user** | PQ signatures on presence records (§5), the ~35 KB budget, storage estimates | Record size becomes a real cost and the PQ exception needs revisiting | Field data or a credible usage model for ceremonies per user over time. |
| **A6** | **Verification querying will be routine** | The shared-identity defence (§7.3) | The attack succeeds most of the time. Also enforceable via client default rather than assumed | Client telemetry or a user study showing automatic verifier queries actually occur at the assumed rate in normal use. |
| **A7** | **Infra operators persist for years with their records** | Reliability weighting (§16.6), verification answerability (§7.4.3), the whole "infra silence is not excusable" argument | Infra participation stops being more evidentially durable and §7.4.3's distinction collapses | Longitudinal operator-retention data showing infrastructure identities and their records persist materially longer than ordinary participants. |
| **A8** | **Attackers cannot parallelise physical presence** | A1's teeth (§17.3) | Paid participants, simultaneous ceremonies and colluding witnesses would defeat it | An attack model or experiment establishing the practical concurrency limit on paid/colluding humans performing simultaneous ceremonies. |
| **A9** | **Cross-device face matching FRR is a few percent** | The aggregation rule and threshold design (§7.4.4); also §20.1 | A materially higher rate makes false accusation common; a much lower one makes single-negative policies safe | A specified face matcher, threshold, capture protocol, device mix, and dataset measuring cross-device false-reject rate. |
| **A10** | **Real social graphs are sparse, so honest operators never achieve theoretical packing** | The claim in §17.2 that attacker economics are *worse* than 1:1 for defenders | The cost symmetry becomes exactly 1:1, weakening §17.2's conclusion | Empirical social-graph degree/branching data and packing simulations comparing honest topology with the theoretical f-bounded maximum. |
| **A11** | **Ordinary users will meet face to face often enough for the network to work, and will treat it as worth doing rather than as friction to route around** | The whole Sybil defence, which only works if the mechanism is used (§7.1), and §6.1.1's requirement, which has no unattested fallback to degrade into. **Restated 2026-09-05**: encouraging frequent face-to-face meeting is a design aim [author], so this is not a friction the design hopes users absorb but the behaviour it is built to produce | Users route around the mechanism — meeting rarely, reusing old counterparties, or transferring rather than meeting wherever §6.1.1 allows it, and the face-to-face grounding thins to whatever the transfer path can carry. Note this is the honest-user mirror of A1: A1 prices the attacker's meetings, this one asks whether ordinary people take theirs | A field/usability study measuring ceremony completion, repetition, abandonment, and workarounds under realistic incentives. |
| **A12** | **The facts a composition attack would yield are already obtainable about the target user from existing sources** | The acceptance of composition risk in §19.1 as a *matched* rather than *new* exposure | For any user whose baseline exposure is lower — deliberate minimisers, activists, dissidents, people hiding from someone. The trade is not the one described, and the network creates exposure rather than matching it. **Known false for part of the population**; the question is whether it holds for the intended one | A comparative exposure study for the intended population showing the same relationship/location facts are normally obtainable from existing sources. |
| **A13** | **Users successfully retain and back up their transaction archive** | The second-factor property (§10.2), portability of history to a new subnet (§16.7), and the value of the chain at all | If archives are routinely lost, users arrive at every new subnet as fresh identities and accumulated standing becomes non-portable in practice. The security property survives; the usability does not, and §13.7.1's backup design becomes the whole story | Observed backup/restore success rates for the target client population, including device loss and migration. |
| **A14** | Reconstruction against a binary-output matcher needs **thousands to tens of thousands of queries** | §7.4.1's whole oracle-hardening argument — ceremony binding turns that count into weeks of staged meetings | Also §20.1. If the true count is orders lower, ceremony binding and rate limits do not price the attack out | A cited or reproduced hill-climbing/reconstruction attack against the specified binary matcher, with measured query complexity. |
| **A15** | **"A substantial minority"** of connections fall back to TURN relay | §16.6's infra economics via §12.6.3's direct path | Also §20.1. Two mobile peers behind CGNAT is worst case *and* common case here; a high fraction collapses the cost model | NAT/CGNAT traversal telemetry for the target device/network mix, measuring direct-path versus TURN fallback rates. |
| **A16** | Randomised motion prompts **constitute** a liveness check | Presentation-attack resistance for the whole ceremony (§7.1) | Also §20.1. They are *inputs* to a PAD system; if no algorithm delivers the property, print and replay attacks pass | Presentation-attack-detection evaluation of the specified randomized-prompt algorithm against print, replay, deepfake/generated-video, and relay attacks. |
| **A17** | **Face entropy is low enough** that fuzzy commitments have weak margins | Used to *reject* a mechanism that would retain verification capability without retaining biometrics (Appendix B.1) | Also §20.1. If false, the whole retention design could change. This is the only assumption used to close off an alternative rather than support a choice | A concrete face representation, entropy estimate, fuzzy-commitment/fuzzy-extractor construction, and security analysis including helper-data leakage. |
| **A18** | Ageing is modest for adults, severe for minors, **substantial in 24 months** | The two-year capture retention tier (§7.5.1) | Also §20.1 | Longitudinal face-recognition data by age group under the intended capture conditions, with error versus elapsed time. |
| **A19** | Infra costs **~$20/month retail, ~$5–7 marginal to an attacker** | §16.6's operator pricing and §17.3's static-addressing leg | Also §20.1 | A dated bill of materials and cloud-price survey for the assumed infra configuration, plus attacker consolidation/discount assumptions. |
| **A21** | **Patrons will administer resources.** Hold a connection to a wider system, host an instance, bind roles, carry availability | §11.0.1's federation pattern, and through it every resource application larger than one neighbourhood | The resource-layer sibling of A11: A11 says users tolerate ceremony friction, this says operators tolerate administration. If false, applications stay local or route around the network, and if they route around it, §1.2's product argument goes with them | Operator research or deployment evidence measuring willingness and ability to host/federate resources, bind roles, and maintain availability. |
| **A22** | Protocol-defined high-importance transactions occur **far less often than once per 100 seconds per user** | The capacity argument under the control-plane topology (§1) | If ordinary use is transaction-heavier than assumed, apex load ceases to be dominated by churn and A3 fails with it | A representative transaction-rate model or deployment telemetry for protocol-defined high-importance transactions per user. |
| **A34** | **A former patron's countersignature is adequate evidence in place of a meeting between the two new parties** | §6.1.1's second route, and with it every lateral and vertical shift (§6.2.3) — the alternative being that each such move costs a fresh ceremony | If a countersignature is materially weaker than a meeting, transfers become the cheap route into a horizon that §6.1.1 closed for unattested adoptions, and an attacker who controls one patron walks its subordinates into subnets that never met them. **Replaces A23, whose premise — that proof of presence is optional — §6.1.1 removed on 2026-09-05**; the question moved rather than closed, from *what is an unattested edge worth* to *what is a vouched-for one worth* | A threat/evidence analysis or experiment comparing former-patron countersignatures with fresh meetings for resistance to malicious transfer/adoption. |
| **A24** | A user accumulates **a few hundred archive records per decade** | The claim that post-quantum archive size is operationally insignificant (§5) | An order of magnitude more makes the archive a storage and bandwidth problem, not a rounding error | A workload/lifecycle model or field data for archive-record accumulation per user per decade. |
| **A25** | An envelope past **~400 KB** is prohibitive rather than merely large | Keeping embedded evidence classical instead of hybrid (§5.1) | If that size is tolerable, the simpler uniform rule (everything hybrid) becomes available | Implementation benchmarks showing the operational/UX/memory/bandwidth cost curve around 400 KB envelopes on target devices and transports. |
| **A26** | A fuzzed profile discriminating **~99% of humans** is the right privacy/utility point | The biometric query representation (§7.5) | Too specific and it approaches court-grade evidence; too vague and verification stops working | A privacy/utility evaluation showing why the chosen fuzzed-profile operating point (~99% discrimination) is appropriate for both matching and privacy. |
| **A27** | A normal presence record carries **~10 logical signers, around 8 of them witnesses** | The ~35 KB record figure and the storage arithmetic that follows from it (§8.2) | Witness counts are a social artifact of how ceremonies actually run. Materially more witnesses and the size estimate moves proportionally; materially fewer and the corroboration the record claims is thinner than modelled | Observed ceremony data giving the distribution of logical signers and witnesses per record. |
| **A28** | **Package authors' incentive runs toward breadth** in permission defaults | The requirement that templates be inspectable in the predicate language (`resource-requirements.md` §7.3) | An economic claim about a party's motivation, asserted rather than argued. If authors default conservatively instead, the inspectability requirement addresses a problem that does not arise | Package-author behavior data (or an ecosystem study) showing default permission requests tend toward breadth. |
| **A29** | **Fabricating a whole fictitious graph is easy**, and convincing synthetic histories are within reach of a motivated party | §1.2.1's deniability property — if fabrication is harder than assumed, correlated evidence is *more* probative and the deniability shrinks | Asserted from the observation that a fabricator holds every key. Nobody has built one, and a graph that survives an evaluator with local reach may be considerably harder than one that survives a distant reader | A working synthetic-subnet construction and red-team evaluation showing the effort required to create convincing fictitious histories. |
| **A30** | **A remote evaluator cannot distinguish a synthesised subnet from a real one** | The same property, and §1.2.2's claim that the discount falls hardest on the classes least able to defeat it | Follows from A29 plus the absence of cold lookup. Untested against an evaluator applying statistical structure analysis rather than key-checking | A blinded evaluator study or statistical distinguishability analysis comparing synthetic and real subnets under the evaluator's allowed observations. |
| **A31** | **To an attacker accountable to no evidentiary standard, cryptographic attestation adds nothing** | §1.2.2's three-class taxonomy, and the conclusion that on-device encryption is the whole defence against that class | A claim about how such parties actually decide, asserted rather than observed. If signed evidence does shift their behaviour, the archive's non-repudiability costs more than recorded | Evidence about how the stated class of decision-makers treats signed versus unsigned evidence in actual decisions or policy. |
| **A32** | **A compromised resource leaks its own data, not the owner's archive** | The credential gateway instead of a scoped archive-read API (§11, `resource-requirements.md` §1) | Also §20.1. A confinement conclusion over every interface and side channel; if false, the read-surface question §11 claims to dissolve returns [author, 2026-09-02] | A complete interface/side-channel confinement analysis and adversarial test showing resource compromise cannot reach owner archive/state beyond resource data. |
| **A33** | **A relying party's clock is good enough to decide whether a staple has expired** | Which key a relying party addresses as current under §12.6.5, and with it the claim that short credential lifetimes do revocation's work beyond the horizon. Local timing is unconstrained and other mechanisms use it too — a witness declines a ceremony dated far from its own clock (`light-client-requirements.md` §1.2), so what singles this one out is not that it consults a clock but that a **security** decision turns on the answer | An adversary who can skew a victim's clock backwards extends a stolen credential's life at will, and expiry stops doing revocation's work for that victim. The error is asymmetric: skew forward costs only availability, so the dangerous direction is the one an attacker prefers, and a party has no second source to check its clock against | A clock threat model and target-device measurements establishing acceptable skew/tamper resistance for expiry decisions, including backward-skew attacks. |

**§20.1 and §20.2 are orthogonal registers.**
§20.1 records what is **unsourced**; §20.2 records what is **load-bearing**. A
claim can be both, and **A14–A19 and A32 are.** Which makes those seven the
highest-priority items in the document, since they are simultaneously
unvalidated and structural.
