...
check(len(re.findall(r'\| TR\d+ ', ms)) == 18, 'messages: eighteen session traces')
qc = frame_objs[12][1]
...
check(len([e for e in corpus['entries'] if e['class'] == 'trace']) == 18
      and len([e for e in corpus['entries'] if e['class'] == 'context']) == 5,
      'corpus: eighteen traces and five contexts')
