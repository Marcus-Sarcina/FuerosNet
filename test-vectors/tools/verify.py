...
check(len(re.findall(r'\| TR\d+ ', ms)) == 19, 'messages: nineteen session traces')
qc = frame_objs[12][1]
...
check(len([e for e in corpus['entries'] if e['class'] == 'trace']) == 19
      and len([e for e in corpus['entries'] if e['class'] == 'context']) == 5,
      'corpus: nineteen traces and five contexts')
